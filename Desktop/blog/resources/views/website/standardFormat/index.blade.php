@extends('layouts.layout')
@section('content')
<div class="blog-post-9 pt-20">
        <div class="container">
            <div class="inner-page-breadcrumb-wrapper mb-20">
                <ul>
                    <li><a href="index.html">Home</a></li>
                    <li><a href="gaming.html">Gaming</a></li>
                    <li>Gameplay Gazette: News from the Gaming Front.</li>
                </ul>
            </div>
            <div class="row g-lg-4 gy-5">
                <div class="col-lg-8">
                    <div class="post-thump">
                        <img src="assets/image/standard/thump-image1.png" alt="">
                    </div>
                    <ul class="post-meta">
                        <li>
                            <a href="editor-profile.html">Caleb Benjamin</a>
                        </li>
                        <li>
                            <a class="publish-date" href="life-style.html">27 Jan, 2024</a>
                        </li>
                    </ul>
                    <h1>Infinite Insert Your Journey into Gaming Infinity</h1>
                    <div class="row justify-content-center">
                        <div class="col-lg-10 mb-60">
                            <div class="blog-content">
                                <p>"Infinite Odyssey: Your Journey into Gaming Infinity" is a groundbreaking gaming
                                    experience that transcends traditional boundaries, offering players an infinite
                                    universe of exploration, discovery, and adventure. </p>
                                <p>In this game, players embark on an <a class="text" href="#">Epic Journey</a> through
                                    a vast and
                                    ever-expanding multiverse of interconnected worlds, each with its own unique
                                    challenges, mysteries, and wonders.players have the opportunity to establish their
                                    own thriving economies, trading goods and services with other pioneers and NPCs
                                    (non-player characters). Whether they're farming crops, crafting rare items, or
                                    running their own shops and businesses, players can earn wealth and prestige in the
                                    world of Pixel Pioneers.</p>
                                <h3>Endless Exploration</h3>
                                <p>Players are free to chart their own course through the vast expanse of the
                                    multiverse, uncovering hidden secrets, forging alliances strange civilizations.</p>
                                <p>The narrative of Infinite Odyssey is shaped by the actions and choices of the
                                    players, with branching storylines, dynamic events, and emergent gameplay</p>
                                <blockquote>
                                    <p>The universe of <a class="text" href="#">"Infinite Odyssey"</a> is filled with
                                        countless
                                        challenges and rewards for players to discover. From hidden treasures and rare
                                        artifacts to epic boss battles and world-altering events, there's always
                                        something new and exciting waiting to be found.</p>
                                    <div class="author-name">
                                        <span>Dr. Samuel Nathan</span>
                                    </div>
                                </blockquote>
                            </div>
                        </div>
                    </div>
                    <div class="post-thump-image">
                        <img src="assets/image/standard/thum-image2.png" alt="">
                    </div>
                    <div class="row justify-content-center">
                        <div class="col-lg-10 mb-40">
                            <div class="blog-content2">
                                <span>"Infinite Odyssey: Your Journey into Gaming Infinity" </span>
                                <h3>Customization and Progression</h3>
                                <p><a href="#"><mark>Offers extensive customization options,</mark></a> allowing players
                                    to create and customize
                                    their
                                    own
                                    characters, vehicles, and equipment.</p>
                                <p>As they progress through the game, they'll earn experience points, unlock new
                                    abilities,
                                    and
                                    discover powerful artifacts that enhance their skills and abilities.</p>
                                <ul>
                                    <li>
                                        <svg width="14" height="14" viewbox="0 0 14 14" fill="none" xmlns="http://www.w3.org/2000/svg">
                                            <g>
                                                <path d="M14 7C14 8.85652 13.2625 10.637 11.9497 11.9497C10.637 13.2625 8.85652 14 7 14C5.14348 14 3.36301 13.2625 2.05025 11.9497C0.737498 10.637 0 8.85652 0 7C0 5.14348 0.737498 3.36301 2.05025 2.05025C3.36301 0.737498 5.14348 0 7 0C8.85652 0 10.637 0.737498 11.9497 2.05025C13.2625 3.36301 14 5.14348 14 7ZM10.5262 4.34875C10.4637 4.28647 10.3893 4.23743 10.3074 4.20456C10.2256 4.1717 10.1379 4.15569 10.0497 4.15749C9.96144 4.15928 9.87449 4.17885 9.79401 4.21502C9.71352 4.25119 9.64116 4.30322 9.58125 4.368L6.54238 8.23987L4.711 6.40763C4.5866 6.29171 4.42206 6.2286 4.25204 6.2316C4.08203 6.2346 3.91982 6.30347 3.79958 6.42371C3.67934 6.54394 3.61047 6.70615 3.60747 6.87617C3.60447 7.04618 3.66758 7.21072 3.7835 7.33513L6.09875 9.65125C6.16112 9.71351 6.23539 9.76257 6.31714 9.7955C6.39888 9.82843 6.48642 9.84456 6.57453 9.84293C6.66264 9.84129 6.74952 9.82193 6.82998 9.78599C6.91045 9.75005 6.98285 9.69828 7.04287 9.63375L10.5359 5.2675C10.655 5.14369 10.7207 4.97812 10.7191 4.80634C10.7175 4.63456 10.6485 4.47027 10.5271 4.34875H10.5262Z" fill="#D9D9D9"></path>
                                            </g>
                                        </svg>
                                        The universe of "Infinite Odyssey" is filled with countless challenges.
                                    </li>
                                    <li>
                                        <svg width="14" height="14" viewbox="0 0 14 14" fill="none" xmlns="http://www.w3.org/2000/svg">
                                            <g>
                                                <path d="M14 7C14 8.85652 13.2625 10.637 11.9497 11.9497C10.637 13.2625 8.85652 14 7 14C5.14348 14 3.36301 13.2625 2.05025 11.9497C0.737498 10.637 0 8.85652 0 7C0 5.14348 0.737498 3.36301 2.05025 2.05025C3.36301 0.737498 5.14348 0 7 0C8.85652 0 10.637 0.737498 11.9497 2.05025C13.2625 3.36301 14 5.14348 14 7ZM10.5262 4.34875C10.4637 4.28647 10.3893 4.23743 10.3074 4.20456C10.2256 4.1717 10.1379 4.15569 10.0497 4.15749C9.96144 4.15928 9.87449 4.17885 9.79401 4.21502C9.71352 4.25119 9.64116 4.30322 9.58125 4.368L6.54238 8.23987L4.711 6.40763C4.5866 6.29171 4.42206 6.2286 4.25204 6.2316C4.08203 6.2346 3.91982 6.30347 3.79958 6.42371C3.67934 6.54394 3.61047 6.70615 3.60747 6.87617C3.60447 7.04618 3.66758 7.21072 3.7835 7.33513L6.09875 9.65125C6.16112 9.71351 6.23539 9.76257 6.31714 9.7955C6.39888 9.82843 6.48642 9.84456 6.57453 9.84293C6.66264 9.84129 6.74952 9.82193 6.82998 9.78599C6.91045 9.75005 6.98285 9.69828 7.04287 9.63375L10.5359 5.2675C10.655 5.14369 10.7207 4.97812 10.7191 4.80634C10.7175 4.63456 10.6485 4.47027 10.5271 4.34875H10.5262Z" fill="#D9D9D9"></path>
                                            </g>
                                        </svg>
                                        Here's always something new and exciting waiting to be found.
                                    </li>
                                </ul>
                                <h3>Cross-Platform Connectivity</h3>
                                <p>Immerse yourself in interactive features, including polls, quizzes, and
                                    reader-submitted
                                    content. Become an active part of the gaming conversation and share your gaming
                                    experiences with the community. </p>
                            </div>

                        </div>
                    </div>
                    <a href="https://themeforest.net/item/axleo-digital-agency-creative-portfolio-template/51144673" class="add-image">
                        <img src="assets/image/standard/Ads-Banner.png" alt="">
                    </a>
                    <div class="blog-tag">
                        <div class="author-name">
                            <h6>Posted by <a href="editor-profile.html">Robert Kcarery</a></h6>
                        </div>
                        <div class="tag-items">
                            <h6>Categorized:</h6>
                            <ul>
                                <li><a href="life-style.html">Gaming Tips,</a></li>
                                <li><a href="life-style.html">Competition,</a></li>
                                <li><a href="life-style.html">Chanllenge</a></li>
                            </ul>
                        </div>
                    </div>
                    <div class="post-btn">
                        <div class="privious-post-btn">
                            <a href="#">
                                <svg class="arrow" width="11" height="11" viewbox="0 0 11 11" xmlns="http://www.w3.org/2000/svg">
                                    <path d="M10 10L1 1M1 1C3 1.375 7.375 2.125 10 1M1 1C1.375 2.875 2.125 7 1 10" stroke="white" stroke-width="1.5" stroke-linecap="round"></path>
                                </svg>
                                PREVIOUS POST</a>
                        </div>
                        <div class="next-post-btn">
                            <a href="#">
                                <svg class="arrow" width="10" height="10" viewbox="0 0 10 10" xmlns="http://www.w3.org/2000/svg">
                                    <path d="M1 9L9 1M9 1C7.22222 1.33333 3.33333 2 1 1M9 1C8.66667 2.66667 8 6.33333 9 9" stroke="#191919" stroke-width="1.5" stroke-linecap="round"></path>
                                </svg>
                                NEXT POST</a>
                        </div>
                    </div>
                    <div class="share-post-area">
                        <div class="row">
                            <div class="col-lg-12">
                                <div class="share-post-content">
                                    <h3>Share This Post</h3>
                                    <ul>
                                        <li>
                                            <a class="icon" href="https://www.facebook.com/">
                                                <svg width="6" height="12" viewbox="0 0 6 12" xmlns="http://www.w3.org/2000/svg">
                                                    <path d="M3.81526 11.2483V6.46735H5.42818L5.66793 4.59543H3.81526V3.4031C3.81526 2.86293 3.96576 2.4931 4.74101 2.4931H5.72334V0.824182C5.24538 0.77296 4.76495 0.748228 4.28426 0.750099C2.85859 0.750099 1.87976 1.62043 1.87976 3.21818V4.59193H0.277344V6.46385H1.88326V11.2483H3.81526Z"></path>
                                                </svg>
                                            </a>
                                        </li>
                                        <li>
                                            <a class="icon" href="https://x.com/">
                                                <svg width="14" height="14" viewbox="0 0 14 14" xmlns="http://www.w3.org/2000/svg">
                                                    <g>
                                                        <path d="M11.025 0.65625H13.1722L8.48225 6.0305L14 13.3438H9.68012L6.2965 8.9075L2.42462 13.3438H0.2765L5.29287 7.595L0 0.65625H4.43013L7.48825 4.71012L11.025 0.65625ZM10.2725 12.0557H11.4625L3.78262 1.87687H2.50688L10.2725 12.0557Z"></path>
                                                    </g>
                                                </svg>
                                            </a>
                                        </li>
                                        <li>
                                            <a class="icon" href="https://www.linkedin.com/">
                                                <svg width="14" height="14" viewbox="0 0 14 14" xmlns="http://www.w3.org/2000/svg">
                                                    <path d="M2.90719 4.1972C3.61209 4.1972 4.18353 3.62576 4.18353 2.92086C4.18353 2.21597 3.61209 1.64453 2.90719 1.64453C2.20229 1.64453 1.63086 2.21597 1.63086 2.92086C1.63086 3.62576 2.20229 4.1972 2.90719 4.1972Z"></path>
                                                    <path d="M5.38752 5.16523V12.2463H7.5861V8.74457C7.5861 7.82057 7.75994 6.92573 8.9056 6.92573C10.0355 6.92573 10.0495 7.98215 10.0495 8.8029V12.2469H12.2493V8.36365C12.2493 6.45615 11.8386 4.99023 9.60911 4.99023C8.53869 4.99023 7.82119 5.57765 7.52777 6.13357H7.49802V5.16523H5.38752ZM1.80469 5.16523H4.00677V12.2463H1.80469V5.16523Z"></path>
                                                </svg>

                                            </a>
                                        </li>
                                        <li>
                                            <a class="icon" href="https://www.instagram.com/">
                                                <svg width="14" height="14" viewbox="0 0 14 14" xmlns="http://www.w3.org/2000/svg">
                                                    <path d="M12.2191 4.84492C12.2132 4.40312 12.1305 3.96571 11.9747 3.55226C11.8396 3.20351 11.6332 2.88679 11.3687 2.62233C11.1043 2.35786 10.7875 2.15147 10.4388 2.01634C10.0307 1.86313 9.59948 1.78029 9.16362 1.77134C8.60246 1.74626 8.42454 1.73926 7.00004 1.73926C5.57554 1.73926 5.39296 1.73926 4.83587 1.77134C4.40022 1.78036 3.96924 1.8632 3.56129 2.01634C3.21249 2.15138 2.89572 2.35773 2.63124 2.62221C2.36677 2.88669 2.16041 3.20346 2.02537 3.55226C1.87186 3.96008 1.7892 4.39116 1.78096 4.82684C1.75587 5.38859 1.74829 5.56651 1.74829 6.99101C1.74829 8.41551 1.74829 8.59751 1.78096 9.15517C1.78971 9.59151 1.87196 10.022 2.02537 10.4309C2.16064 10.7796 2.36715 11.0963 2.63171 11.3606C2.89628 11.625 3.21308 11.8313 3.56187 11.9663C3.96871 12.1256 4.39976 12.2144 4.83646 12.2288C5.39821 12.2538 5.57612 12.2614 7.00062 12.2614C8.42512 12.2614 8.60771 12.2614 9.16479 12.2288C9.60063 12.2202 10.0318 12.1375 10.44 11.9843C10.7886 11.8491 11.1053 11.6426 11.3697 11.3782C11.6341 11.1137 11.8406 10.7971 11.9759 10.4484C12.1293 10.0401 12.2115 9.60959 12.2203 9.17267C12.2454 8.61151 12.253 8.43359 12.253 7.00851C12.2518 5.58401 12.2518 5.40317 12.2191 4.84492ZM6.99654 9.68484C5.50671 9.68484 4.29979 8.47792 4.29979 6.98809C4.29979 5.49826 5.50671 4.29134 6.99654 4.29134C7.71176 4.29134 8.39769 4.57546 8.90343 5.0812C9.40917 5.58694 9.69329 6.27287 9.69329 6.98809C9.69329 7.70331 9.40917 8.38924 8.90343 8.89498C8.39769 9.40072 7.71176 9.68484 6.99654 9.68484ZM9.80062 4.82042C9.71802 4.8205 9.63622 4.80429 9.55989 4.77271C9.48356 4.74114 9.41421 4.69482 9.3558 4.63641C9.29739 4.57801 9.25108 4.50865 9.2195 4.43233C9.18793 4.356 9.17171 4.27419 9.17179 4.19159C9.17179 4.10905 9.18805 4.02732 9.21964 3.95106C9.25122 3.8748 9.29752 3.80551 9.35589 3.74714C9.41425 3.68878 9.48354 3.64248 9.5598 3.61089C9.63606 3.57931 9.71779 3.56305 9.80033 3.56305C9.88287 3.56305 9.96461 3.57931 10.0409 3.61089C10.1171 3.64248 10.1864 3.68878 10.2448 3.74714C10.3031 3.80551 10.3494 3.8748 10.381 3.95106C10.4126 4.02732 10.4289 4.10905 10.4289 4.19159C10.4289 4.53926 10.1477 4.82042 9.80062 4.82042Z"></path>
                                                    <path d="M6.99589 8.73983C7.96336 8.73983 8.74764 7.95554 8.74764 6.98808C8.74764 6.02061 7.96336 5.23633 6.99589 5.23633C6.02843 5.23633 5.24414 6.02061 5.24414 6.98808C5.24414 7.95554 6.02843 8.73983 6.99589 8.73983Z"></path>
                                                </svg>
                                            </a>
                                        </li>
                                    </ul>
                                    <div class="input-area" id="inviteCode">
                                        <input id="link-input" value="https://www.egenstheme.com/" readonly="">
                                        <div id="copy-link-icon">
                                            <svg width="16" height="16" viewbox="0 0 16 16" xmlns="http://www.w3.org/2000/svg">
                                                <path fill-rule="evenodd" clip-rule="evenodd" d="M6.97324 10.9956C6.99524 11.0066 7.02724 11.0066 7.02724 11.0066V11.0286C7.06024 11.0286 7.12424 10.9846 7.12424 10.9846L8.29028 9.82042L8.32328 9.78845C8.35314 9.76404 8.37852 9.73462 8.39828 9.70151C8.42028 9.65854 8.43028 9.61457 8.40828 9.5716C8.38828 9.52863 8.35528 9.48566 8.31228 9.48566C7.87035 9.39014 7.46545 9.1691 7.14624 8.84912C6.76572 8.47567 6.53189 7.97821 6.48723 7.44712C6.45456 7.1118 6.49673 6.77341 6.61069 6.45634C6.72464 6.13926 6.90755 5.85138 7.14624 5.61343L10.2003 2.55962C11.0634 1.69624 12.5744 1.69624 13.4384 2.55962C13.8704 3.0023 14.1084 3.57389 14.1084 4.18845C14.1084 4.80301 13.8704 5.3856 13.4384 5.81729L12.0464 7.1983C12.0306 7.21556 12.0193 7.23645 12.0136 7.25912C12.0078 7.28178 12.0077 7.30551 12.0134 7.32821C12.1434 7.79088 12.2184 8.27653 12.2184 8.76218C12.2184 8.99901 12.2084 9.20387 12.1754 9.37674C12.1754 9.4307 12.1974 9.49566 12.2514 9.51664C12.2768 9.532 12.3066 9.53828 12.3361 9.53447C12.3655 9.53066 12.3928 9.51697 12.4134 9.49566L14.7775 7.14434C16.4075 5.50451 16.4075 2.85041 14.7775 1.22158C13.9921 0.43928 12.9283 0 11.8194 0C10.7104 0 9.6467 0.43928 8.86129 1.22158L5.80721 4.27539C5.5372 4.5452 5.32219 4.83699 5.16019 5.12778L5.14719 5.13778C5.13719 5.14577 5.12718 5.15276 5.12718 5.15976C4.79591 5.74751 4.61115 6.40632 4.58852 7.0805C4.56589 7.75467 4.70606 8.42438 4.99718 9.03299C5.20319 9.45269 5.46219 9.83042 5.80721 10.1762C6.14221 10.5209 6.54123 10.8017 6.97324 10.9956ZM1.23107 14.7709C2.0501 15.5803 3.12013 15.99 4.18816 15.99L4.15616 16C5.22419 16 6.30422 15.5903 7.11424 14.7819L10.1683 11.7291C10.727 11.1773 11.1165 10.4777 11.2914 9.7125L11.3564 9.23784C11.3774 9.12992 11.3774 9.02199 11.3774 8.91407V8.48338C11.3774 8.37446 11.3674 8.26654 11.3454 8.1806C11.3354 8.06168 11.3134 7.96475 11.2914 7.86782C11.2664 7.74735 11.234 7.62854 11.1944 7.51208C11.0008 6.87575 10.652 6.29748 10.1793 5.82928C9.84282 5.49624 9.45228 5.22256 9.0243 5.01986C8.9813 4.98788 8.87329 5.04184 8.87329 5.04184L7.70726 6.20601C7.67526 6.23899 7.63226 6.28196 7.59926 6.34591C7.57826 6.37889 7.57826 6.42186 7.59926 6.46582C7.62126 6.50879 7.65326 6.54077 7.69626 6.54077C8.13927 6.62671 8.53828 6.84256 8.86229 7.16632C9.29431 7.60901 9.53231 8.19159 9.52131 8.82713C9.51031 9.21685 9.41331 9.59359 9.2293 9.91635C9.17794 10.0094 9.11658 10.0965 9.0463 10.1762C9.0033 10.2411 8.9493 10.3161 8.86229 10.402L5.80721 13.4548C5.37361 13.8759 4.79279 14.1115 4.18816 14.1115C3.58352 14.1115 3.00271 13.8759 2.56911 13.4548C2.13942 13.0215 1.89858 12.436 1.89909 11.826C1.89909 11.2114 2.1371 10.6288 2.56911 10.1972L3.96215 8.80615C3.99415 8.76318 4.00515 8.70922 3.99415 8.66625C3.77779 8.01315 3.71483 7.31903 3.81015 6.6377C3.81488 6.60938 3.81008 6.58029 3.79651 6.55498C3.78294 6.52966 3.76136 6.50956 3.73515 6.4978C3.69214 6.46483 3.58414 6.51879 3.58414 6.51879L1.23007 8.85911C0.838368 9.24512 0.527785 9.70543 0.316558 10.213C0.105332 10.7206 -0.00228147 11.2653 3.66695e-05 11.815C3.66695e-05 12.9252 0.44305 13.9834 1.23007 14.7699L1.23107 14.7709Z"></path>
                                            </svg>
                                        </div>
                                        <span id="copy-text" class="copy-text"></span>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="subscribe-section">
                        <h3>Subscribe To Get Update
                            Latest Blog Post</h3>
                        <div class="input-area">
                            <input type="email" placeholder="Enter Email" class="">
                            <button type="submit">
                                <svg width="16" height="16" viewbox="0 0 16 16" xmlns="http://www.w3.org/2000/svg">
                                    <path d="M15.9647 0.685806C16.0011 0.594942 16.01 0.495402 15.9904 0.399526C15.9707 0.303649 15.9233 0.215653 15.8541 0.146447C15.7849 0.0772403 15.6969 0.0298668 15.601 0.0101994C15.5052 -0.0094681 15.4056 -0.000564594 15.3147 0.0358061L0.76775 5.85481H0.76675L0.31475 6.03481C0.22914 6.06895 0.154635 6.1261 0.0994654 6.19994C0.0442956 6.27377 0.0106078 6.36142 0.00212322 6.4532C-0.00636132 6.54497 0.0106876 6.63731 0.0513867 6.72001C0.0920859 6.8027 0.154851 6.87254 0.23275 6.92181L0.64275 7.18181L0.64375 7.18381L5.63875 10.3618L8.81675 15.3568L8.81875 15.3588L9.07875 15.7688C9.12817 15.8464 9.19805 15.9089 9.28068 15.9493C9.36332 15.9897 9.45551 16.0066 9.54711 15.998C9.63871 15.9894 9.72617 15.9558 9.79985 15.9007C9.87354 15.8456 9.9306 15.7712 9.96475 15.6858L15.9647 0.685806ZM14.1317 2.57581L6.63775 10.0698L6.42275 9.73181C6.38336 9.66978 6.33078 9.6172 6.26875 9.57781L5.93075 9.36281L13.4247 1.86881L14.6027 1.39781L14.1327 2.57581H14.1317Z">
                                    </path>
                                </svg>
                            </button>
                        </div>
                        <p>
                            <svg width="12" height="12" viewbox="0 0 12 12" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <g clip-path="url(#clip0_786_352)">
                                    <path d="M12 6C12 7.5913 11.3679 9.11742 10.2426 10.2426C9.11742 11.3679 7.5913 12 6 12C4.4087 12 2.88258 11.3679 1.75736 10.2426C0.632141 9.11742 0 7.5913 0 6C0 4.4087 0.632141 2.88258 1.75736 1.75736C2.88258 0.632141 4.4087 0 6 0C7.5913 0 9.11742 0.632141 10.2426 1.75736C11.3679 2.88258 12 4.4087 12 6ZM9.0225 3.7275C8.96893 3.67411 8.90514 3.63208 8.83495 3.60391C8.76476 3.57574 8.68961 3.56202 8.61399 3.56356C8.53838 3.5651 8.46385 3.58187 8.39486 3.61288C8.32588 3.64388 8.26385 3.68848 8.2125 3.744L5.60775 7.06275L4.038 5.49225C3.93137 5.39289 3.79033 5.3388 3.64461 5.34137C3.49888 5.34394 3.35984 5.40297 3.25678 5.50603C3.15372 5.60909 3.09469 5.74813 3.09212 5.89386C3.08955 6.03958 3.14364 6.18062 3.243 6.28725L5.2275 8.2725C5.28096 8.32586 5.34462 8.36791 5.41469 8.39614C5.48475 8.42437 5.55979 8.43819 5.63531 8.43679C5.71083 8.43539 5.7853 8.4188 5.85427 8.38799C5.92324 8.35719 5.9853 8.31281 6.03675 8.2575L9.03075 4.515C9.13282 4.40887 9.18921 4.26696 9.18781 4.11972C9.1864 3.97248 9.12732 3.83166 9.02325 3.7275H9.0225Z" fill="#545454"></path>
                                </g>
                                <defs>
                                    <clippath id="clip0_786_352">
                                        <rect width="12" height="12" fill="white"></rect>
                                    </clippath>
                                </defs>
                            </svg>
                            No Credit Card Required
                        </p>
                    </div>
                    <div class="blog-post-area pt-90">
                        <h6>You May Also Like</h6>
                        <span class="line-break3"></span>
                        <span class="line-break"></span>
                        <div class="row gy-5">
                            <div class="col-md-6">
                                <div class="blog-card2 two">
                                    <div class="blog-card-img-wrap">
                                        <a href="standard-formate.html"><img src="assets/image/standard/stander-blog-img1.png" alt=""></a>
                                        <ul>
                                            <li><a class="category" href="life-style.html">Gaming Joke</a></li>
                                            <li><span class="lock">
                                                    <svg width="14" height="14" viewbox="0 0 14 14" xmlns="http://www.w3.org/2000/svg">
                                                        <path d="M10.9375 5.14062H9.95211V2.89215C9.95211 1.29741 8.62734 0 6.99899 0C5.37064 0 4.04586 1.29741 4.04586 2.89215V5.14062H3.0625C2.15786 5.14062 1.42188 5.87661 1.42188 6.78125V12.3594C1.42188 13.264 2.15786 14 3.0625 14H10.9375C11.8421 14 12.5781 13.264 12.5781 12.3594V6.78125C12.5781 5.87661 11.8421 5.14062 10.9375 5.14062ZM5.13961 2.89215C5.13961 1.9005 5.97373 1.09375 6.99899 1.09375C8.02424 1.09375 8.85836 1.9005 8.85836 2.89215V5.14062H5.13961V2.89215ZM11.4844 12.3594C11.4844 12.6609 11.239 12.9062 10.9375 12.9062H3.0625C2.76095 12.9062 2.51562 12.6609 2.51562 12.3594V6.78125C2.51562 6.4797 2.76095 6.23438 3.0625 6.23438H10.9375C11.239 6.23438 11.4844 6.4797 11.4844 6.78125V12.3594Z">
                                                        </path>
                                                        <path d="M7 7.82031C6.44123 7.82031 5.98828 8.27326 5.98828 8.83203C5.98828 9.18881 6.17312 9.50223 6.45211 9.68237V10.8828C6.45211 11.1848 6.69695 11.4297 6.99899 11.4297C7.301 11.4297 7.54586 11.1848 7.54586 10.8828V9.68365C7.82597 9.50373 8.01172 9.18969 8.01172 8.83203C8.01172 8.27326 7.55877 7.82031 7 7.82031Z">
                                                        </path>
                                                    </svg>
                                                </span>
                                            </li>
                                        </ul>
                                    </div>
                                    <div class="blog-content">
                                        <ul class="blog-meta">
                                            <li>
                                                <a href="editor-profile.html">Robert Kcarery</a>
                                            </li>
                                            <li>
                                                <a class="publish-date" href="life-style.html">01 March, 2024</a>
                                            </li>
                                        </ul>
                                        <h4><a href="standard-formate.html">Pixel Paragon Where Legends are Forged.</a>
                                        </h4>
                                    </div>
                                </div>
                            </div>
                            <div class="col-md-6">
                                <div class="blog-card2 two">
                                    <div class="blog-card-img-wrap">
                                        <a href="standard-formate.html"><img src="assets/image/standard/stander-blog-img2.png" alt=""></a>
                                        <ul>
                                            <li><a class="category" href="life-style.html">PC Gaming</a></li>
                                        </ul>
                                    </div>
                                    <div class="blog-content">
                                        <ul class="blog-meta">
                                            <li>
                                                <a href="editor-profile.html">Dr. Samuel Nathan</a>
                                            </li>
                                            <li>
                                                <a class="publish-date" href="life-style.html">01 Feb, 2024</a>
                                            </li>
                                        </ul>
                                        <h4><a href="standard-formate.html">Chronicle Controller Tales of Gaming
                                                Triumphs</a></h4>
                                    </div>
                                </div>
                            </div>
                            <div class="col-md-6">
                                <div class="blog-card2 two">
                                    <div class="blog-card-img-wrap">
                                        <a href="standard-formate.html"><img src="assets/image/standard/stander-blog-img3.png" alt=""></a>
                                        <ul>
                                            <li><a class="category" href="life-style.html">Gaming Tips</a></li>
                                        </ul>
                                    </div>
                                    <div class="blog-content">
                                        <ul class="blog-meta">
                                            <li>
                                                <a href="editor-profile.html">Dr. Michael Patrick</a>
                                            </li>
                                            <li>
                                                <a class="publish-date" href="life-style.html">02 Feb, 2024</a>
                                            </li>
                                        </ul>
                                        <h4><a href="standard-formate.html">Console Connoisseur: Unveiling Gaming
                                                Wonders</a></h4>
                                    </div>
                                </div>
                            </div>
                            <div class="col-md-6">
                                <div class="blog-card2 two">
                                    <div class="blog-card-img-wrap">
                                        <a href="standard-formate.html"><img src="assets/image/standard/stander-blog-img4.png" alt=""></a>
                                        <ul>
                                            <li><a class="category" href="life-style.html">Offline</a></li>
                                        </ul>
                                    </div>
                                    <div class="blog-content">
                                        <ul class="blog-meta">
                                            <li>
                                                <a href="editor-profile.html">Robert Kcarery</a>
                                            </li>
                                            <li>
                                                <a class="publish-date" href="life-style.html">05 Feb, 2024</a>
                                            </li>
                                        </ul>
                                        <h4><a href="standard-formate.html">Virtual Vista: Exploring Boundless Game Worlds</a></h4>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="row pt-100">
                        <div class="col-lg-12">
                            <div class="comment-area mb-60">
                                <div class="comment-title">
                                    <h4>Comments (02)</h4>
                                </div>
                                <ul class="comment">
                                    <li>
                                        <div class="single-comment-area">
                                            <div class="author-img">
                                                <img src="assets/image/standard/comment-img1.png" alt="">
                                            </div>
                                            <div class="comment-content">
                                                <div class="author-name-deg">
                                                    <h6>Mr. Bowmik Haldar,</h6>
                                                    <span>02 March, 2024</span>
                                                </div>
                                                <p>Great! Have to learn more.</p>
                                                <div class="replay-btn">
                                                    <svg xmlns="http://www.w3.org/2000/svg" width="14" height="11" viewbox="0 0 14 11">
                                                        <path d="M8.55126 1.11188C8.52766 1.10118 8.50182 1.09676 8.47612 1.09903C8.45042 1.1013 8.42569 1.11018 8.40419 1.12486C8.3827 1.13954 8.36513 1.15954 8.35311 1.18304C8.34109 1.20653 8.335 1.23276 8.33539 1.25932V2.52797C8.33539 2.67388 8.2791 2.81381 8.17889 2.91698C8.07868 3.02016 7.94277 3.07812 7.80106 3.07812C7.08826 3.07812 5.64984 3.08362 4.27447 3.98257C3.2229 4.66916 2.14783 5.9191 1.50129 8.24735C2.59132 7.16575 3.83632 6.57929 4.92635 6.2679C5.59636 6.07737 6.28492 5.96444 6.97926 5.93121C7.26347 5.91835 7.54815 5.92129 7.83205 5.94001H7.84594L7.85129 5.94111L7.80106 6.48906L7.85449 5.94111C7.98638 5.95476 8.10864 6.01839 8.19751 6.11966C8.28638 6.22092 8.33553 6.35258 8.33539 6.48906V7.75771C8.33539 7.87654 8.45294 7.95136 8.55126 7.90515L12.8088 4.67796C12.8233 4.66692 12.8383 4.65664 12.8537 4.64715C12.8769 4.63278 12.8962 4.61245 12.9095 4.58816C12.9229 4.56386 12.9299 4.53643 12.9299 4.50851C12.9299 4.4806 12.9229 4.45316 12.9095 4.42887C12.8962 4.40458 12.8769 4.38425 12.8537 4.36988C12.8382 4.36039 12.8233 4.35011 12.8088 4.33907L8.55126 1.11188ZM7.26673 7.02381C7.19406 7.02381 7.11391 7.02711 7.02842 7.03041C6.56462 7.05242 5.92342 7.12504 5.21169 7.32859C3.79464 7.7335 2.11684 8.65116 1.00115 10.7175C0.940817 10.8291 0.844683 10.9155 0.729224 10.9621C0.613765 11.0087 0.486168 11.0124 0.368304 10.9728C0.250441 10.9331 0.149648 10.8525 0.0831985 10.7447C0.0167484 10.6369 -0.011219 10.5086 0.0040884 10.3819C0.499949 6.29981 2.01959 4.15202 3.70167 3.05391C5.03215 2.18467 6.40218 2.01743 7.26673 1.98552V1.25932C7.26663 1.03273 7.32593 0.810317 7.43839 0.615545C7.55084 0.420773 7.71227 0.260866 7.90565 0.152696C8.09902 0.0445258 8.31717 -0.00789584 8.53707 0.000962485C8.75698 0.00982081 8.97048 0.0796305 9.15506 0.203025L13.4233 3.43792C13.5998 3.55133 13.7453 3.7091 13.8462 3.8964C13.9471 4.08369 14 4.29434 14 4.50851C14 4.72269 13.9471 4.93333 13.8462 5.12063C13.7453 5.30792 13.5998 5.4657 13.4233 5.57911L9.15506 8.814C8.97048 8.9374 8.75698 9.00721 8.53707 9.01607C8.31717 9.02492 8.09902 8.9725 7.90565 8.86433C7.71227 8.75616 7.55084 8.59626 7.43839 8.40148C7.32593 8.20671 7.26663 7.9843 7.26673 7.75771V7.02381Z">
                                                        </path>
                                                    </svg>
                                                    Reply (01)
                                                </div>
                                            </div>
                                        </div>
                                        <ul class="comment-replay">
                                            <li>
                                                <div class="single-comment-area">
                                                    <div class="author-img">
                                                        <img src="assets/image/standard/comment-img2.png" alt="">
                                                    </div>
                                                    <div class="comment-content">
                                                        <div class="author-name-deg">
                                                            <h6>Jacoline Juie,</h6>
                                                            <span>05 March, 2024</span>
                                                        </div>
                                                        <p>Take Love Brothers!</p>
                                                    </div>
                                                </div>
                                            </li>
                                        </ul>
                                    </li>
                                    <li>
                                        <div class="single-comment-area">
                                            <div class="author-img">
                                                <img src="assets/image/standard/comment-img3.png" alt="">
                                            </div>
                                            <div class="comment-content">
                                                <div class="author-name-deg">
                                                    <h6>Mr. Bowmik Haldar,</h6>
                                                    <span>02 March, 2024</span>
                                                </div>
                                                <p>Masterclass article!</p>
                                                <div class="replay-btn">
                                                    <svg xmlns="http://www.w3.org/2000/svg" width="14" height="11" viewbox="0 0 14 11">
                                                        <path d="M8.55126 1.11188C8.52766 1.10118 8.50182 1.09676 8.47612 1.09903C8.45042 1.1013 8.42569 1.11018 8.40419 1.12486C8.3827 1.13954 8.36513 1.15954 8.35311 1.18304C8.34109 1.20653 8.335 1.23276 8.33539 1.25932V2.52797C8.33539 2.67388 8.2791 2.81381 8.17889 2.91698C8.07868 3.02016 7.94277 3.07812 7.80106 3.07812C7.08826 3.07812 5.64984 3.08362 4.27447 3.98257C3.2229 4.66916 2.14783 5.9191 1.50129 8.24735C2.59132 7.16575 3.83632 6.57929 4.92635 6.2679C5.59636 6.07737 6.28492 5.96444 6.97926 5.93121C7.26347 5.91835 7.54815 5.92129 7.83205 5.94001H7.84594L7.85129 5.94111L7.80106 6.48906L7.85449 5.94111C7.98638 5.95476 8.10864 6.01839 8.19751 6.11966C8.28638 6.22092 8.33553 6.35258 8.33539 6.48906V7.75771C8.33539 7.87654 8.45294 7.95136 8.55126 7.90515L12.8088 4.67796C12.8233 4.66692 12.8383 4.65664 12.8537 4.64715C12.8769 4.63278 12.8962 4.61245 12.9095 4.58816C12.9229 4.56386 12.9299 4.53643 12.9299 4.50851C12.9299 4.4806 12.9229 4.45316 12.9095 4.42887C12.8962 4.40458 12.8769 4.38425 12.8537 4.36988C12.8382 4.36039 12.8233 4.35011 12.8088 4.33907L8.55126 1.11188ZM7.26673 7.02381C7.19406 7.02381 7.11391 7.02711 7.02842 7.03041C6.56462 7.05242 5.92342 7.12504 5.21169 7.32859C3.79464 7.7335 2.11684 8.65116 1.00115 10.7175C0.940817 10.8291 0.844683 10.9155 0.729224 10.9621C0.613765 11.0087 0.486168 11.0124 0.368304 10.9728C0.250441 10.9331 0.149648 10.8525 0.0831985 10.7447C0.0167484 10.6369 -0.011219 10.5086 0.0040884 10.3819C0.499949 6.29981 2.01959 4.15202 3.70167 3.05391C5.03215 2.18467 6.40218 2.01743 7.26673 1.98552V1.25932C7.26663 1.03273 7.32593 0.810317 7.43839 0.615545C7.55084 0.420773 7.71227 0.260866 7.90565 0.152696C8.09902 0.0445258 8.31717 -0.00789584 8.53707 0.000962485C8.75698 0.00982081 8.97048 0.0796305 9.15506 0.203025L13.4233 3.43792C13.5998 3.55133 13.7453 3.7091 13.8462 3.8964C13.9471 4.08369 14 4.29434 14 4.50851C14 4.72269 13.9471 4.93333 13.8462 5.12063C13.7453 5.30792 13.5998 5.4657 13.4233 5.57911L9.15506 8.814C8.97048 8.9374 8.75698 9.00721 8.53707 9.01607C8.31717 9.02492 8.09902 8.9725 7.90565 8.86433C7.71227 8.75616 7.55084 8.59626 7.43839 8.40148C7.32593 8.20671 7.26663 7.9843 7.26673 7.75771V7.02381Z">
                                                        </path>
                                                    </svg>
                                                    Reply (01)
                                                </div>
                                            </div>
                                        </div>
                                        <ul class="comment-replay two">
                                            <li>
                                                <div class="single-comment-area">
                                                    <div class="author-img">
                                                        <img src="assets/image/standard/comment-img2.png" alt="">
                                                    </div>
                                                    <div class="comment-content">
                                                        <div class="author-name-deg">
                                                            <h6>Jacoline Juie,</h6>
                                                            <span>05 March, 2024</span>
                                                        </div>
                                                        <p>Take Love Brothers!</p>
                                                    </div>
                                                </div>
                                            </li>
                                        </ul>
                                    </li>
                                </ul>
                            </div>
                        </div>
                    </div>
                   <div class="row">
                    <div class="col-lg-12">
                        <div class="inquiry-form ">
                            <div class="title">
                                <h4>Leave Your Comment:</h4>
                            </div>
                            <form>
                                <div class="row">
                                    <div class="col-md-6">
                                        <div class="form-inner mb-20">
                                            <label>Your Name* :</label>
                                            <input type="text" placeholder="Jackson Mile">
                                        </div>
                                    </div>
                                    <div class="col-md-6">
                                        <div class="form-inner mb-20">
                                            <label>Your Email* :</label>
                                            <input type="email" placeholder="example@gamil.com">
                                        </div>
                                    </div>
                                    <div class="col-md-12">
                                        <div class="form-inner mb-15">
                                            <label>Your Comments*</label>
                                            <textarea placeholder="Write Something..."></textarea>
                                        </div>
                                    </div>
                                    <div class="col-md-12">
                                        <div class="form-check">
                                            <input class="form-check-input" type="checkbox" value="" id="contactCheck">
                                            <label class="form-check-label" for="contactCheck">
                                                Please save my name, email address for the next time I
                                                comment.
                                            </label>
                                        </div>
                                    </div>
                                </div>
                                <div class="form-inner">
                                    <button class="primary-btn1" data-text="Post Comment" type="submit">
                                        <span> <svg class="arrow" width="10" height="10" viewbox="0 0 10 10" xmlns="http://www.w3.org/2000/svg">
                                                <path d="M1 9L9 1M9 1C7.22222 1.33333 3.33333 2 1 1M9 1C8.66667 2.66667 8 6.33333 9 9" stroke="#191919" stroke-width="1.5" stroke-linecap="round"></path>
                                            </svg>
                                            SUBMIT COMMENT</span>
                                    </button>
                                </div>
                            </form>
                        </div>
                    </div>
                   </div>
                </div>
                <div class="col-lg-4">
                    <div class="life-style-sidebar">
                        <div class="sidebar-widget driscription">
                            <div class="header-logo pb-15">
                                <a href="index.html"><img alt="image" class="img-fluid dark" src="assets/image/index-img/icon/dark-logo.svg"></a>
                                <a href="index.html"><img alt="image" class="img-fluid light" src="assets/image/index-img/icon/light-logo.svg"></a>
                            </div>
                            <p><span>Welcome!</span> Feel free to mix and match these subcategories based on your interests and the preferences of your target audience. Health &amp; Wellness is a diverse niche.</p>
                            <div class="counter-area">
                                <ul>
                                    <li class="single-counter">
                                        <a href="https://www.facebook.com/">
                                            <div class="icon">
                                                <svg width="6" height="12" viewbox="0 0 6 12" xmlns="http://www.w3.org/2000/svg">
                                                    <path d="M3.81526 11.2482V6.46723H5.42818L5.66793 4.59531H3.81526V3.40298C3.81526 2.86281 3.96576 2.49298 4.74101 2.49298H5.72334V0.82406C5.24538 0.772838 4.76495 0.748106 4.28426 0.749977C2.85859 0.749977 1.87976 1.62031 1.87976 3.21806V4.59181H0.277344V6.46373H1.88326V11.2482H3.81526Z"></path>
                                                </svg>
                                            </div>
                                            <div class="number">
                                                <h5 class="counter">222</h5>
                                                <span>K</span>
                                            </div>
                                        </a>
                                    </li>
                                    <li class="single-counter">
                                        <a href="https://youtube.com/">
                                            <div class="icon">
                                                <svg width="14" height="11" viewbox="0 0 14 11" xmlns="http://www.w3.org/2000/svg">
                                                    <path d="M7.04462 0.749146H7.1225C7.84175 0.751771 11.4861 0.778021 12.4688 1.04227C12.7658 1.12292 13.0365 1.28012 13.2538 1.49815C13.471 1.71619 13.6273 1.98743 13.7069 2.28477C13.7952 2.61727 13.8574 3.0574 13.8994 3.51152L13.9081 3.60252L13.9274 3.83002L13.9344 3.92102C13.9913 4.72077 13.9983 5.46977 13.9991 5.6334V5.69902C13.9983 5.86877 13.9904 6.66852 13.9274 7.50152L13.9204 7.5934L13.9125 7.6844C13.8687 8.1849 13.804 8.6819 13.7069 9.04765C13.6273 9.34499 13.471 9.61623 13.2538 9.83426C13.0365 10.0523 12.7658 10.2095 12.4688 10.2901C11.4537 10.5631 7.59587 10.5824 7.06125 10.5833H6.937C6.66663 10.5833 5.54838 10.578 4.37587 10.5378L4.22712 10.5325L4.151 10.529L4.00138 10.5229L3.85175 10.5168C2.8805 10.4739 1.95563 10.4048 1.5295 10.2893C1.23255 10.2087 0.961931 10.0516 0.74467 9.83376C0.527409 9.61588 0.371108 9.34482 0.291375 9.04765C0.19425 8.68277 0.1295 8.1849 0.08575 7.6844L0.07875 7.59252L0.07175 7.50152C0.028318 6.90863 0.0043884 6.31448 0 5.72002L0 5.6124C0.00175 5.42427 0.00875 4.77415 0.056 4.05665L0.062125 3.96652L0.06475 3.92102L0.07175 3.83002L0.091 3.60252L0.09975 3.51152C0.14175 3.0574 0.203875 2.6164 0.29225 2.28477C0.371849 1.98743 0.528091 1.71619 0.74536 1.49815C0.96263 1.28012 1.23332 1.12292 1.53038 1.04227C1.9565 0.928521 2.88137 0.85852 3.85262 0.81477L4.00138 0.808645L4.15188 0.803396L4.22712 0.800771L4.37675 0.794646C5.2095 0.767874 6.04258 0.752997 6.87575 0.750021L7.04462 0.749146ZM5.6 3.5579V7.77365L9.23738 5.66665L5.6 3.5579Z"></path>
                                                </svg>
                                            </div>
                                            <div class="number">
                                                <h5 class="counter">222</h5>
                                                <span>K</span>
                                            </div>
                                        </a>
                                    </li>
                                    <li class="single-counter">
                                        <a href="https://www.instagram.com/">
                                            <div class="icon">
                                                <svg width="12" height="12" viewbox="0 0 12 12" xmlns="http://www.w3.org/2000/svg">
                                                    <path d="M11.2189 3.84492C11.213 3.40312 11.1303 2.96571 10.9745 2.55226C10.8393 2.20351 10.6329 1.88679 10.3685 1.62233C10.104 1.35786 9.78729 1.15147 9.43855 1.01634C9.03041 0.863133 8.59923 0.78029 8.16338 0.771341C7.60221 0.746258 7.4243 0.739258 5.9998 0.739258C4.5753 0.739258 4.39271 0.739258 3.83563 0.771341C3.39997 0.780356 2.969 0.863198 2.56105 1.01634C2.21225 1.15138 1.89548 1.35773 1.631 1.62221C1.36652 1.88669 1.16017 2.20346 1.02513 2.55226C0.871615 2.96008 0.788952 3.39116 0.780714 3.82684C0.75563 4.38859 0.748047 4.56651 0.748047 5.99101C0.748047 7.41551 0.748047 7.59751 0.780714 8.15517C0.789464 8.59151 0.871713 9.02201 1.02513 9.43092C1.16039 9.77962 1.3669 10.0963 1.63147 10.3606C1.89603 10.625 2.21283 10.8313 2.56163 10.9663C2.96847 11.1256 3.39951 11.2144 3.83621 11.2288C4.39796 11.2538 4.57588 11.2614 6.00038 11.2614C7.42488 11.2614 7.60746 11.2614 8.16455 11.2288C8.60039 11.2202 9.03159 11.1375 9.43971 10.9843C9.78837 10.8491 10.105 10.6426 10.3695 10.3782C10.6339 10.1137 10.8403 9.79708 10.9756 9.44842C11.129 9.04009 11.2113 8.60959 11.22 8.17267C11.2451 7.61151 11.2527 7.43359 11.2527 6.00851C11.2515 4.58401 11.2515 4.40317 11.2189 3.84492V3.84492ZM5.9963 8.68484C4.50646 8.68484 3.29955 7.47792 3.29955 5.98809C3.29955 4.49826 4.50646 3.29134 5.9963 3.29134C6.71152 3.29134 7.39745 3.57546 7.90319 4.0812C8.40893 4.58694 8.69305 5.27287 8.69305 5.98809C8.69305 6.70331 8.40893 7.38924 7.90319 7.89498C7.39745 8.40072 6.71152 8.68484 5.9963 8.68484V8.68484ZM8.80038 3.82042C8.71778 3.8205 8.63597 3.80429 8.55965 3.77271C8.48332 3.74114 8.41397 3.69482 8.35556 3.63641C8.29715 3.57801 8.25083 3.50865 8.21926 3.43233C8.18768 3.356 8.17147 3.27419 8.17155 3.19159C8.17155 3.10905 8.1878 3.02732 8.21939 2.95106C8.25098 2.8748 8.29728 2.80551 8.35564 2.74714C8.41401 2.68878 8.4833 2.64248 8.55956 2.61089C8.63581 2.57931 8.71755 2.56305 8.80009 2.56305C8.88263 2.56305 8.96436 2.57931 9.04062 2.61089C9.11688 2.64248 9.18617 2.68878 9.24453 2.74714C9.3029 2.80551 9.3492 2.8748 9.38079 2.95106C9.41237 3.02732 9.42863 3.10905 9.42863 3.19159C9.42863 3.53926 9.14746 3.82042 8.80038 3.82042Z"></path>
                                                    <path d="M5.99589 7.73983C6.96336 7.73983 7.74764 6.95554 7.74764 5.98808C7.74764 5.02061 6.96336 4.23633 5.99589 4.23633C5.02843 4.23633 4.24414 5.02061 4.24414 5.98808C4.24414 6.95554 5.02843 7.73983 5.99589 7.73983Z"></path>
                                                </svg>
                                            </div>
                                            <div class="number">
                                                <h5 class="counter">222</h5>
                                                <span>K</span>
                                            </div>
                                        </a>
                                    </li>
                                </ul>
                            </div>
                        </div>
                        <div class="sidebar-widget featured-post">
                            <h6>Featured Post</h6>
                            <div class="recent-post mb-20">
                                <div class="recent-post-img">
                                    <a href="standard-formate.html"><img src="assets/image/life-style/sidebar-img1.png" alt=""></a>
                                </div>
                                <div class="recent-post-content">
                                    <a href="life-style.html">05 January, 2024</a>
                                    <h5><a href="standard-formate.html">A Guide to Better Sleep Habits.</a></h5>
                                </div>
                            </div>
                            <div class="recent-post mb-20">
                                <div class="recent-post-img">
                                    <a href="standard-formate.html"><img src="assets/image/life-style/sidebar-img2.png" alt=""></a>
                                </div>
                                <div class="recent-post-content">
                                    <a href="life-style.html">05 January, 2024</a>
                                    <h5><a href="standard-formate.html">A Guide to Better Sleep Habits.</a></h5>
                                </div>
                            </div>
                            <div class="recent-post mb-20">
                                <div class="recent-post-img">
                                    <a href="standard-formate.html"><img src="assets/image/life-style/sidebar-img3.png" alt=""></a>
                                </div>
                                <div class="recent-post-content">
                                    <a href="life-style.html">05 January, 2024</a>
                                    <h5><a href="standard-formate.html">A Guide to Better Sleep Habits.</a></h5>
                                </div>
                            </div>
                            <div class="recent-post mb-20">
                                <div class="recent-post-img">
                                    <a href="standard-formate.html"><img src="assets/image/life-style/sidebar-img4.png" alt=""></a>
                                </div>
                                <div class="recent-post-content">
                                    <a href="life-style.html">05 January, 2024</a>
                                    <h5><a href="standard-formate.html">A Guide to Better Sleep Habits.</a></h5>
                                </div>
                            </div>
                        </div>
                        <div class="sidebar-widget discover-post">
                            <h6 class="discover-title">Discover By Tags
                                <svg class="arrow" width="10" height="10" viewbox="0 0 10 10" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <path d="M1 9L9 1M9 1C7.22222 1.33333 3.33333 2 1 1M9 1C8.66667 2.66667 8 6.33333 9 9" stroke="white" stroke-width="1.5" stroke-linecap="round"></path>
                                </svg>
                            </h6>
                            <ul class="category">
                                <li>
                                    <a href="life-style.html">Beauty (10)</a>
                                </li>
                                <li>
                                    <a href="life-style.html">Gaming Tips (05)</a>
                                </li>
                                <li>
                                    <a href="life-style.html">Fashion (02)</a>
                                </li>
                                <li>
                                    <a href="life-style.html">Tech (03)</a>
                                </li>
                                <li>
                                    <a href="life-style.html">Parenting (03)</a>
                                </li>
                                <li>
                                    <a href="life-style.html">Health (03)</a>
                                </li>
                                <li>
                                    <a href="life-style.html">Events (04)</a>
                                </li>
                                <li>
                                    <a href="life-style.html">Art (03)</a>
                                </li>
                                <li>
                                    <a href="life-style.html">Food (06)</a>
                                </li>
                                <li>
                                    <a href="life-style.html">Travel (02)</a>
                                </li>
                            </ul>
                        </div>
                        <div class="sidebar-widget about-section mb-40">
                            <ul class="category">
                                <li>
                                    <a href="about-us.html">About Us</a>
                                </li>
                                <li>
                                    <a href="terms%26condition.html">Terms & Conditions</a>
                                </li>
                                <li>
                                    <a href="#">Support</a>
                                </li>
                                <li>
                                    <a href="privacy-policy.html">Privacy Policy</a>
                                </li>
                            </ul>
                        </div>
                        <a href="https://themeforest.net/item/axleo-digital-agency-creative-portfolio-template/51144673" class="sidebar-add-image">
                            <img src="assets/image/life-style/Sidebar-ads.png" alt="">
                        </a>
                    </div>
                </div>
            </div>
        </div>
    </div>
    <!-- blog section ends here -->
    <!-- Galleary section strats here -->
    <div class="gallery-section pt-100">
        <div class="container">
            <div class="row">
                <div class="col-lg-12">
                    <div class="sub-title mb-30">
                        <h6>
                            <svg width="16" height="16" viewbox="0 0 16 16" xmlns="http://www.w3.org/2000/svg">
                                <g clip-path="url(#clip0_956_810)">
                                    <path d="M8 0C5.829 0 5.556 0.01 4.703 0.048C3.85 0.088 3.269 0.222 2.76 0.42C2.22584 0.620326 1.74214 0.935398 1.343 1.343C0.935398 1.74214 0.620326 2.22584 0.42 2.76C0.222 3.268 0.087 3.85 0.048 4.7C0.01 5.555 0 5.827 0 8.001C0 10.173 0.01 10.445 0.048 11.298C0.088 12.15 0.222 12.731 0.42 13.24C0.625 13.766 0.898 14.212 1.343 14.657C1.787 15.102 2.233 15.376 2.759 15.58C3.269 15.778 3.849 15.913 4.701 15.952C5.555 15.99 5.827 16 8 16C10.173 16 10.444 15.99 11.298 15.952C12.149 15.912 12.732 15.778 13.241 15.58C13.7748 15.3795 14.2581 15.0645 14.657 14.657C15.102 14.212 15.375 13.766 15.58 13.24C15.777 12.731 15.912 12.15 15.952 11.298C15.99 10.445 16 10.173 16 8C16 5.827 15.99 5.555 15.952 4.701C15.912 3.85 15.777 3.268 15.58 2.76C15.3797 2.22584 15.0646 1.74214 14.657 1.343C14.2579 0.935398 13.7742 0.620326 13.24 0.42C12.73 0.222 12.148 0.087 11.297 0.048C10.443 0.01 10.172 0 7.998 0H8ZM7.283 1.442H8.001C10.137 1.442 10.39 1.449 11.233 1.488C12.013 1.523 12.437 1.654 12.719 1.763C13.092 1.908 13.359 2.082 13.639 2.362C13.919 2.642 14.092 2.908 14.237 3.282C14.347 3.563 14.477 3.987 14.512 4.767C14.551 5.61 14.559 5.863 14.559 7.998C14.559 10.133 14.551 10.387 14.512 11.23C14.477 12.01 14.346 12.433 14.237 12.715C14.1078 13.0619 13.9032 13.3758 13.638 13.634C13.358 13.914 13.092 14.087 12.718 14.232C12.438 14.342 12.014 14.472 11.233 14.508C10.39 14.546 10.137 14.555 8.001 14.555C5.865 14.555 5.611 14.546 4.768 14.508C3.988 14.472 3.565 14.342 3.283 14.232C2.93583 14.1033 2.6216 13.899 2.363 13.634C2.09727 13.3756 1.89233 13.0614 1.763 12.714C1.654 12.433 1.523 12.009 1.488 11.229C1.45 10.386 1.442 10.133 1.442 7.996C1.442 5.859 1.45 5.608 1.488 4.765C1.524 3.985 1.654 3.561 1.764 3.279C1.909 2.906 2.083 2.639 2.363 2.359C2.643 2.079 2.909 1.906 3.283 1.761C3.565 1.651 3.988 1.521 4.768 1.485C5.506 1.451 5.792 1.441 7.283 1.44V1.442ZM12.271 2.77C12.1449 2.77 12.0201 2.79483 11.9036 2.84308C11.7872 2.89132 11.6813 2.96203 11.5922 3.05118C11.503 3.14032 11.4323 3.24615 11.3841 3.36262C11.3358 3.4791 11.311 3.60393 11.311 3.73C11.311 3.85607 11.3358 3.9809 11.3841 4.09738C11.4323 4.21385 11.503 4.31968 11.5922 4.40882C11.6813 4.49797 11.7872 4.56868 11.9036 4.61692C12.0201 4.66517 12.1449 4.69 12.271 4.69C12.5256 4.69 12.7698 4.58886 12.9498 4.40882C13.1299 4.22879 13.231 3.98461 13.231 3.73C13.231 3.47539 13.1299 3.23121 12.9498 3.05118C12.7698 2.87114 12.5256 2.77 12.271 2.77ZM8.001 3.892C7.45607 3.8835 6.91489 3.98349 6.40898 4.18614C5.90306 4.3888 5.44251 4.69007 5.05415 5.07242C4.66579 5.45478 4.35736 5.91057 4.14684 6.41326C3.93632 6.91595 3.8279 7.4555 3.8279 8.0005C3.8279 8.5455 3.93632 9.08505 4.14684 9.58774C4.35736 10.0904 4.66579 10.5462 5.05415 10.9286C5.44251 11.3109 5.90306 11.6122 6.40898 11.8149C6.91489 12.0175 7.45607 12.1175 8.001 12.109C9.07954 12.0922 10.1082 11.6519 10.865 10.8833C11.6217 10.1146 12.0459 9.07917 12.0459 8.0005C12.0459 6.92183 11.6217 5.88641 10.865 5.11775C10.1082 4.34909 9.07954 3.90883 8.001 3.892ZM8.001 5.333C8.35124 5.333 8.69804 5.40198 9.02162 5.53601C9.34519 5.67004 9.6392 5.86649 9.88685 6.11415C10.1345 6.3618 10.331 6.65581 10.465 6.97938C10.599 7.30296 10.668 7.64976 10.668 8C10.668 8.35023 10.599 8.69704 10.465 9.02062C10.331 9.34419 10.1345 9.6382 9.88685 9.88585C9.6392 10.1335 9.34519 10.33 9.02162 10.464C8.69804 10.598 8.35124 10.667 8.001 10.667C7.29367 10.667 6.61531 10.386 6.11515 9.88585C5.61499 9.38569 5.334 8.70733 5.334 8C5.334 7.29267 5.61499 6.61431 6.11515 6.11415C6.61531 5.61399 7.29367 5.333 8.001 5.333Z"></path>
                                </g>
                                <defs>
                                    <clippath id="clip0_956_810">
                                        <rect width="16" height="16" fill="white"></rect>
                                    </clippath>
                                </defs>
                            </svg>
                            INSTAGRAM FEED
                            <svg class="arrow" width="122" height="6" viewbox="0 0 122 6" xmlns="http://www.w3.org/2000/svg">
                                <path d="M122 3L117 0.113249V5.88675L122 3ZM0 3.5H117.5V2.5H0V3.5Z"></path>
                            </svg>
                        </h6>
                    </div>
                </div>
            </div>
        </div>
        <div class="swiper gallery-slider">
            <div class="swiper-wrapper">
                <div class="swiper-slide">
                    <div class="single-gallery-img">
                        <a href="#" class="view-btn">
                            <svg width="26" height="26" viewbox="0 0 26 26" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <rect width="26" height="26" rx="5" fill="url(#paint0_linear_956_788)"></rect>
                                <path d="M13 6C11.1004 6 10.8615 6.00875 10.1151 6.042C9.36875 6.077 8.86037 6.19425 8.415 6.3675C7.94761 6.54278 7.52437 6.81847 7.17513 7.17513C6.81847 7.52437 6.54278 7.94761 6.3675 8.415C6.19425 8.8595 6.07613 9.36875 6.042 10.1125C6.00875 10.8606 6 11.0986 6 13.0009C6 14.9014 6.00875 15.1394 6.042 15.8857C6.077 16.6313 6.19425 17.1396 6.3675 17.585C6.54688 18.0452 6.78575 18.4355 7.17513 18.8249C7.56363 19.2142 7.95388 19.454 8.41412 19.6325C8.86037 19.8057 9.36788 19.9239 10.1134 19.958C10.8606 19.9913 11.0986 20 13 20C14.9014 20 15.1385 19.9913 15.8857 19.958C16.6304 19.923 17.1405 19.8057 17.5859 19.6325C18.0529 19.4571 18.4759 19.1814 18.8249 18.8249C19.2142 18.4355 19.4531 18.0452 19.6325 17.585C19.8049 17.1396 19.923 16.6313 19.958 15.8857C19.9913 15.1394 20 14.9014 20 13C20 11.0986 19.9913 10.8606 19.958 10.1134C19.923 9.36875 19.8049 8.8595 19.6325 8.415C19.4572 7.94761 19.1815 7.52437 18.8249 7.17513C18.4756 6.81847 18.0524 6.54278 17.585 6.3675C17.1388 6.19425 16.6295 6.07613 15.8849 6.042C15.1376 6.00875 14.9005 6 12.9983 6H13ZM12.3726 7.26175H13.0009C14.8699 7.26175 15.0912 7.26788 15.8289 7.302C16.5114 7.33263 16.8824 7.44725 17.1291 7.54262C17.4555 7.6695 17.6891 7.82175 17.9341 8.06675C18.1791 8.31175 18.3305 8.5445 18.4574 8.87175C18.5536 9.11762 18.6674 9.48863 18.698 10.1711C18.7321 10.9088 18.7391 11.1301 18.7391 12.9983C18.7391 14.8664 18.7321 15.0886 18.698 15.8263C18.6674 16.5087 18.5528 16.8789 18.4574 17.1256C18.3444 17.4292 18.1653 17.7038 17.9332 17.9297C17.6882 18.1747 17.4555 18.3261 17.1283 18.453C16.8833 18.5493 16.5122 18.663 15.8289 18.6945C15.0912 18.7278 14.8699 18.7356 13.0009 18.7356C11.1319 18.7356 10.9096 18.7278 10.172 18.6945C9.4895 18.663 9.11937 18.5493 8.87262 18.453C8.56885 18.3403 8.2939 18.1616 8.06763 17.9297C7.83511 17.7037 7.65579 17.4287 7.54262 17.1248C7.44725 16.8789 7.33263 16.5079 7.302 15.8254C7.26875 15.0878 7.26175 14.8664 7.26175 12.9965C7.26175 11.1266 7.26875 10.907 7.302 10.1694C7.3335 9.48687 7.44725 9.11588 7.5435 8.86913C7.67037 8.54275 7.82262 8.30912 8.06763 8.06413C8.31262 7.81912 8.54537 7.66775 8.87262 7.54088C9.11937 7.44463 9.4895 7.33088 10.172 7.29938C10.8177 7.26963 11.068 7.26088 12.3726 7.26V7.26175ZM16.7371 8.42375C16.6268 8.42375 16.5176 8.44548 16.4157 8.48769C16.3138 8.52991 16.2212 8.59178 16.1432 8.66978C16.0652 8.74778 16.0033 8.84038 15.9611 8.9423C15.9189 9.04421 15.8971 9.15344 15.8971 9.26375C15.8971 9.37406 15.9189 9.48329 15.9611 9.5852C16.0033 9.68712 16.0652 9.77972 16.1432 9.85772C16.2212 9.93572 16.3138 9.99759 16.4157 10.0398C16.5176 10.082 16.6268 10.1038 16.7371 10.1038C16.9599 10.1038 17.1736 10.0153 17.3311 9.85772C17.4886 9.70019 17.5771 9.48653 17.5771 9.26375C17.5771 9.04097 17.4886 8.82731 17.3311 8.66978C17.1736 8.51225 16.9599 8.42375 16.7371 8.42375ZM13.0009 9.4055C12.5241 9.39806 12.0505 9.48555 11.6079 9.66287C11.1652 9.8402 10.7622 10.1038 10.4224 10.4384C10.0826 10.7729 9.81269 11.1717 9.62849 11.6116C9.44428 12.0515 9.34941 12.5236 9.34941 13.0004C9.34941 13.4773 9.44428 13.9494 9.62849 14.3893C9.81269 14.8291 10.0826 15.2279 10.4224 15.5625C10.7622 15.8971 11.1652 16.1607 11.6079 16.338C12.0505 16.5153 12.5241 16.6028 13.0009 16.5954C13.9446 16.5807 14.8447 16.1954 15.5069 15.5228C16.169 14.8503 16.5402 13.9443 16.5402 13.0004C16.5402 12.0566 16.169 11.1506 15.5069 10.478C14.8447 9.80545 13.9446 9.42022 13.0009 9.4055ZM13.0009 10.6664C13.3073 10.6664 13.6108 10.7267 13.8939 10.844C14.177 10.9613 14.4343 11.1332 14.651 11.3499C14.8677 11.5666 15.0396 11.8238 15.1569 12.107C15.2741 12.3901 15.3345 12.6935 15.3345 13C15.3345 13.3065 15.2741 13.6099 15.1569 13.893C15.0396 14.1762 14.8677 14.4334 14.651 14.6501C14.4343 14.8668 14.177 15.0387 13.8939 15.156C13.6108 15.2733 13.3073 15.3336 13.0009 15.3336C12.382 15.3336 11.7884 15.0878 11.3508 14.6501C10.9131 14.2125 10.6673 13.6189 10.6673 13C10.6673 12.3811 10.9131 11.7875 11.3508 11.3499C11.7884 10.9122 12.382 10.6664 13.0009 10.6664Z" fill="white"></path>
                                <defs>
                                    <lineargradient id="paint0_linear_956_788" x1="0" y1="0" x2="25.6176" y2="25.6176" gradientunits="userSpaceOnUse">
                                        <stop offset="0" stop-color="#405DE6"></stop>
                                        <stop offset="0.0928316" stop-color="#3A52C4"></stop>
                                        <stop offset="0.188792" stop-color="#833AB4"></stop>
                                        <stop offset="0.304378" stop-color="#C13584"></stop>
                                        <stop offset="0.444285" stop-color="#E1306C"></stop>
                                        <stop offset="0.608987" stop-color="#FF4545" stop-opacity="0.96"></stop>
                                        <stop offset="0.758196" stop-color="#F56040"></stop>
                                        <stop offset="0.893921" stop-color="#FCAF45"></stop>
                                        <stop offset="1" stop-color="#FFDC80"></stop>
                                    </lineargradient>
                                </defs>
                            </svg>
                            <h6>
                                <span>TRAVEL</span>
                                Instagram
                            </h6>
                        </a>
                        <img src="assets/image/standard/gallery-img1.png" alt="">
                    </div>
                </div>
                <div class="swiper-slide">
                    <div class="single-gallery-img">
                        <a href="#" class="view-btn">
                            <svg width="26" height="26" viewbox="0 0 26 26" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <rect width="26" height="26" rx="5" fill="url(#paint0_linear_956_789)"></rect>
                                <path d="M13 6C11.1004 6 10.8615 6.00875 10.1151 6.042C9.36875 6.077 8.86037 6.19425 8.415 6.3675C7.94761 6.54278 7.52437 6.81847 7.17513 7.17513C6.81847 7.52437 6.54278 7.94761 6.3675 8.415C6.19425 8.8595 6.07613 9.36875 6.042 10.1125C6.00875 10.8606 6 11.0986 6 13.0009C6 14.9014 6.00875 15.1394 6.042 15.8857C6.077 16.6313 6.19425 17.1396 6.3675 17.585C6.54688 18.0452 6.78575 18.4355 7.17513 18.8249C7.56363 19.2142 7.95388 19.454 8.41412 19.6325C8.86037 19.8057 9.36788 19.9239 10.1134 19.958C10.8606 19.9913 11.0986 20 13 20C14.9014 20 15.1385 19.9913 15.8857 19.958C16.6304 19.923 17.1405 19.8057 17.5859 19.6325C18.0529 19.4571 18.4759 19.1814 18.8249 18.8249C19.2142 18.4355 19.4531 18.0452 19.6325 17.585C19.8049 17.1396 19.923 16.6313 19.958 15.8857C19.9913 15.1394 20 14.9014 20 13C20 11.0986 19.9913 10.8606 19.958 10.1134C19.923 9.36875 19.8049 8.8595 19.6325 8.415C19.4572 7.94761 19.1815 7.52437 18.8249 7.17513C18.4756 6.81847 18.0524 6.54278 17.585 6.3675C17.1388 6.19425 16.6295 6.07613 15.8849 6.042C15.1376 6.00875 14.9005 6 12.9983 6H13ZM12.3726 7.26175H13.0009C14.8699 7.26175 15.0912 7.26788 15.8289 7.302C16.5114 7.33263 16.8824 7.44725 17.1291 7.54262C17.4555 7.6695 17.6891 7.82175 17.9341 8.06675C18.1791 8.31175 18.3305 8.5445 18.4574 8.87175C18.5536 9.11762 18.6674 9.48863 18.698 10.1711C18.7321 10.9088 18.7391 11.1301 18.7391 12.9983C18.7391 14.8664 18.7321 15.0886 18.698 15.8263C18.6674 16.5087 18.5528 16.8789 18.4574 17.1256C18.3444 17.4292 18.1653 17.7038 17.9332 17.9297C17.6882 18.1747 17.4555 18.3261 17.1283 18.453C16.8833 18.5493 16.5122 18.663 15.8289 18.6945C15.0912 18.7278 14.8699 18.7356 13.0009 18.7356C11.1319 18.7356 10.9096 18.7278 10.172 18.6945C9.4895 18.663 9.11937 18.5493 8.87262 18.453C8.56885 18.3403 8.2939 18.1616 8.06763 17.9297C7.83511 17.7037 7.65579 17.4287 7.54262 17.1248C7.44725 16.8789 7.33263 16.5079 7.302 15.8254C7.26875 15.0878 7.26175 14.8664 7.26175 12.9965C7.26175 11.1266 7.26875 10.907 7.302 10.1694C7.3335 9.48687 7.44725 9.11588 7.5435 8.86913C7.67037 8.54275 7.82262 8.30912 8.06763 8.06413C8.31262 7.81912 8.54537 7.66775 8.87262 7.54088C9.11937 7.44463 9.4895 7.33088 10.172 7.29938C10.8177 7.26963 11.068 7.26088 12.3726 7.26V7.26175ZM16.7371 8.42375C16.6268 8.42375 16.5176 8.44548 16.4157 8.48769C16.3138 8.52991 16.2212 8.59178 16.1432 8.66978C16.0652 8.74778 16.0033 8.84038 15.9611 8.9423C15.9189 9.04421 15.8971 9.15344 15.8971 9.26375C15.8971 9.37406 15.9189 9.48329 15.9611 9.5852C16.0033 9.68712 16.0652 9.77972 16.1432 9.85772C16.2212 9.93572 16.3138 9.99759 16.4157 10.0398C16.5176 10.082 16.6268 10.1038 16.7371 10.1038C16.9599 10.1038 17.1736 10.0153 17.3311 9.85772C17.4886 9.70019 17.5771 9.48653 17.5771 9.26375C17.5771 9.04097 17.4886 8.82731 17.3311 8.66978C17.1736 8.51225 16.9599 8.42375 16.7371 8.42375ZM13.0009 9.4055C12.5241 9.39806 12.0505 9.48555 11.6079 9.66287C11.1652 9.8402 10.7622 10.1038 10.4224 10.4384C10.0826 10.7729 9.81269 11.1717 9.62849 11.6116C9.44428 12.0515 9.34941 12.5236 9.34941 13.0004C9.34941 13.4773 9.44428 13.9494 9.62849 14.3893C9.81269 14.8291 10.0826 15.2279 10.4224 15.5625C10.7622 15.8971 11.1652 16.1607 11.6079 16.338C12.0505 16.5153 12.5241 16.6028 13.0009 16.5954C13.9446 16.5807 14.8447 16.1954 15.5069 15.5228C16.169 14.8503 16.5402 13.9443 16.5402 13.0004C16.5402 12.0566 16.169 11.1506 15.5069 10.478C14.8447 9.80545 13.9446 9.42022 13.0009 9.4055ZM13.0009 10.6664C13.3073 10.6664 13.6108 10.7267 13.8939 10.844C14.177 10.9613 14.4343 11.1332 14.651 11.3499C14.8677 11.5666 15.0396 11.8238 15.1569 12.107C15.2741 12.3901 15.3345 12.6935 15.3345 13C15.3345 13.3065 15.2741 13.6099 15.1569 13.893C15.0396 14.1762 14.8677 14.4334 14.651 14.6501C14.4343 14.8668 14.177 15.0387 13.8939 15.156C13.6108 15.2733 13.3073 15.3336 13.0009 15.3336C12.382 15.3336 11.7884 15.0878 11.3508 14.6501C10.9131 14.2125 10.6673 13.6189 10.6673 13C10.6673 12.3811 10.9131 11.7875 11.3508 11.3499C11.7884 10.9122 12.382 10.6664 13.0009 10.6664Z" fill="white"></path>
                                <defs>
                                    <lineargradient id="paint0_linear_956_789" x1="0" y1="0" x2="25.6176" y2="25.6176" gradientunits="userSpaceOnUse">
                                        <stop offset="0" stop-color="#405DE6"></stop>
                                        <stop offset="0.0928316" stop-color="#3A52C4"></stop>
                                        <stop offset="0.188792" stop-color="#833AB4"></stop>
                                        <stop offset="0.304378" stop-color="#C13584"></stop>
                                        <stop offset="0.444285" stop-color="#E1306C"></stop>
                                        <stop offset="0.608987" stop-color="#FF4545" stop-opacity="0.96"></stop>
                                        <stop offset="0.758196" stop-color="#F56040"></stop>
                                        <stop offset="0.893921" stop-color="#FCAF45"></stop>
                                        <stop offset="1" stop-color="#FFDC80"></stop>
                                    </lineargradient>
                                </defs>
                            </svg>
                            <h6>
                                <span>TRAVEL</span>
                                Instagram
                            </h6>
                        </a>
                        <img src="assets/image/standard/gallery-img2.png" alt="">
                    </div>
                </div>
                <div class="swiper-slide">
                    <div class="single-gallery-img">
                        <a href="#" class="view-btn">
                            <svg width="26" height="26" viewbox="0 0 26 26" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <rect width="26" height="26" rx="5" fill="url(#paint0_linear_956_790)"></rect>
                                <path d="M13 6C11.1004 6 10.8615 6.00875 10.1151 6.042C9.36875 6.077 8.86037 6.19425 8.415 6.3675C7.94761 6.54278 7.52437 6.81847 7.17513 7.17513C6.81847 7.52437 6.54278 7.94761 6.3675 8.415C6.19425 8.8595 6.07613 9.36875 6.042 10.1125C6.00875 10.8606 6 11.0986 6 13.0009C6 14.9014 6.00875 15.1394 6.042 15.8857C6.077 16.6313 6.19425 17.1396 6.3675 17.585C6.54688 18.0452 6.78575 18.4355 7.17513 18.8249C7.56363 19.2142 7.95388 19.454 8.41412 19.6325C8.86037 19.8057 9.36788 19.9239 10.1134 19.958C10.8606 19.9913 11.0986 20 13 20C14.9014 20 15.1385 19.9913 15.8857 19.958C16.6304 19.923 17.1405 19.8057 17.5859 19.6325C18.0529 19.4571 18.4759 19.1814 18.8249 18.8249C19.2142 18.4355 19.4531 18.0452 19.6325 17.585C19.8049 17.1396 19.923 16.6313 19.958 15.8857C19.9913 15.1394 20 14.9014 20 13C20 11.0986 19.9913 10.8606 19.958 10.1134C19.923 9.36875 19.8049 8.8595 19.6325 8.415C19.4572 7.94761 19.1815 7.52437 18.8249 7.17513C18.4756 6.81847 18.0524 6.54278 17.585 6.3675C17.1388 6.19425 16.6295 6.07613 15.8849 6.042C15.1376 6.00875 14.9005 6 12.9983 6H13ZM12.3726 7.26175H13.0009C14.8699 7.26175 15.0912 7.26788 15.8289 7.302C16.5114 7.33263 16.8824 7.44725 17.1291 7.54262C17.4555 7.6695 17.6891 7.82175 17.9341 8.06675C18.1791 8.31175 18.3305 8.5445 18.4574 8.87175C18.5536 9.11762 18.6674 9.48863 18.698 10.1711C18.7321 10.9088 18.7391 11.1301 18.7391 12.9983C18.7391 14.8664 18.7321 15.0886 18.698 15.8263C18.6674 16.5087 18.5528 16.8789 18.4574 17.1256C18.3444 17.4292 18.1653 17.7038 17.9332 17.9297C17.6882 18.1747 17.4555 18.3261 17.1283 18.453C16.8833 18.5493 16.5122 18.663 15.8289 18.6945C15.0912 18.7278 14.8699 18.7356 13.0009 18.7356C11.1319 18.7356 10.9096 18.7278 10.172 18.6945C9.4895 18.663 9.11937 18.5493 8.87262 18.453C8.56885 18.3403 8.2939 18.1616 8.06763 17.9297C7.83511 17.7037 7.65579 17.4287 7.54262 17.1248C7.44725 16.8789 7.33263 16.5079 7.302 15.8254C7.26875 15.0878 7.26175 14.8664 7.26175 12.9965C7.26175 11.1266 7.26875 10.907 7.302 10.1694C7.3335 9.48687 7.44725 9.11588 7.5435 8.86913C7.67037 8.54275 7.82262 8.30912 8.06763 8.06413C8.31262 7.81912 8.54537 7.66775 8.87262 7.54088C9.11937 7.44463 9.4895 7.33088 10.172 7.29938C10.8177 7.26963 11.068 7.26088 12.3726 7.26V7.26175ZM16.7371 8.42375C16.6268 8.42375 16.5176 8.44548 16.4157 8.48769C16.3138 8.52991 16.2212 8.59178 16.1432 8.66978C16.0652 8.74778 16.0033 8.84038 15.9611 8.9423C15.9189 9.04421 15.8971 9.15344 15.8971 9.26375C15.8971 9.37406 15.9189 9.48329 15.9611 9.5852C16.0033 9.68712 16.0652 9.77972 16.1432 9.85772C16.2212 9.93572 16.3138 9.99759 16.4157 10.0398C16.5176 10.082 16.6268 10.1038 16.7371 10.1038C16.9599 10.1038 17.1736 10.0153 17.3311 9.85772C17.4886 9.70019 17.5771 9.48653 17.5771 9.26375C17.5771 9.04097 17.4886 8.82731 17.3311 8.66978C17.1736 8.51225 16.9599 8.42375 16.7371 8.42375ZM13.0009 9.4055C12.5241 9.39806 12.0505 9.48555 11.6079 9.66287C11.1652 9.8402 10.7622 10.1038 10.4224 10.4384C10.0826 10.7729 9.81269 11.1717 9.62849 11.6116C9.44428 12.0515 9.34941 12.5236 9.34941 13.0004C9.34941 13.4773 9.44428 13.9494 9.62849 14.3893C9.81269 14.8291 10.0826 15.2279 10.4224 15.5625C10.7622 15.8971 11.1652 16.1607 11.6079 16.338C12.0505 16.5153 12.5241 16.6028 13.0009 16.5954C13.9446 16.5807 14.8447 16.1954 15.5069 15.5228C16.169 14.8503 16.5402 13.9443 16.5402 13.0004C16.5402 12.0566 16.169 11.1506 15.5069 10.478C14.8447 9.80545 13.9446 9.42022 13.0009 9.4055ZM13.0009 10.6664C13.3073 10.6664 13.6108 10.7267 13.8939 10.844C14.177 10.9613 14.4343 11.1332 14.651 11.3499C14.8677 11.5666 15.0396 11.8238 15.1569 12.107C15.2741 12.3901 15.3345 12.6935 15.3345 13C15.3345 13.3065 15.2741 13.6099 15.1569 13.893C15.0396 14.1762 14.8677 14.4334 14.651 14.6501C14.4343 14.8668 14.177 15.0387 13.8939 15.156C13.6108 15.2733 13.3073 15.3336 13.0009 15.3336C12.382 15.3336 11.7884 15.0878 11.3508 14.6501C10.9131 14.2125 10.6673 13.6189 10.6673 13C10.6673 12.3811 10.9131 11.7875 11.3508 11.3499C11.7884 10.9122 12.382 10.6664 13.0009 10.6664Z" fill="white"></path>
                                <defs>
                                    <lineargradient id="paint0_linear_956_790" x1="0" y1="0" x2="25.6176" y2="25.6176" gradientunits="userSpaceOnUse">
                                        <stop offset="0" stop-color="#405DE6"></stop>
                                        <stop offset="0.0928316" stop-color="#3A52C4"></stop>
                                        <stop offset="0.188792" stop-color="#833AB4"></stop>
                                        <stop offset="0.304378" stop-color="#C13584"></stop>
                                        <stop offset="0.444285" stop-color="#E1306C"></stop>
                                        <stop offset="0.608987" stop-color="#FF4545" stop-opacity="0.96"></stop>
                                        <stop offset="0.758196" stop-color="#F56040"></stop>
                                        <stop offset="0.893921" stop-color="#FCAF45"></stop>
                                        <stop offset="1" stop-color="#FFDC80"></stop>
                                    </lineargradient>
                                </defs>
                            </svg>
                            <h6>
                                <span>TRAVEL</span>
                                Instagram
                            </h6>
                        </a>
                        <img src="assets/image/standard/gallery-img3.png" alt="">
                    </div>
                </div>
                <div class="swiper-slide">
                    <div class="single-gallery-img">
                        <a href="#" class="view-btn">
                            <svg width="26" height="26" viewbox="0 0 26 26" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <rect width="26" height="26" rx="5" fill="url(#paint0_linear_956_791)"></rect>
                                <path d="M13 6C11.1004 6 10.8615 6.00875 10.1151 6.042C9.36875 6.077 8.86037 6.19425 8.415 6.3675C7.94761 6.54278 7.52437 6.81847 7.17513 7.17513C6.81847 7.52437 6.54278 7.94761 6.3675 8.415C6.19425 8.8595 6.07613 9.36875 6.042 10.1125C6.00875 10.8606 6 11.0986 6 13.0009C6 14.9014 6.00875 15.1394 6.042 15.8857C6.077 16.6313 6.19425 17.1396 6.3675 17.585C6.54688 18.0452 6.78575 18.4355 7.17513 18.8249C7.56363 19.2142 7.95388 19.454 8.41412 19.6325C8.86037 19.8057 9.36788 19.9239 10.1134 19.958C10.8606 19.9913 11.0986 20 13 20C14.9014 20 15.1385 19.9913 15.8857 19.958C16.6304 19.923 17.1405 19.8057 17.5859 19.6325C18.0529 19.4571 18.4759 19.1814 18.8249 18.8249C19.2142 18.4355 19.4531 18.0452 19.6325 17.585C19.8049 17.1396 19.923 16.6313 19.958 15.8857C19.9913 15.1394 20 14.9014 20 13C20 11.0986 19.9913 10.8606 19.958 10.1134C19.923 9.36875 19.8049 8.8595 19.6325 8.415C19.4572 7.94761 19.1815 7.52437 18.8249 7.17513C18.4756 6.81847 18.0524 6.54278 17.585 6.3675C17.1388 6.19425 16.6295 6.07613 15.8849 6.042C15.1376 6.00875 14.9005 6 12.9983 6H13ZM12.3726 7.26175H13.0009C14.8699 7.26175 15.0912 7.26788 15.8289 7.302C16.5114 7.33263 16.8824 7.44725 17.1291 7.54262C17.4555 7.6695 17.6891 7.82175 17.9341 8.06675C18.1791 8.31175 18.3305 8.5445 18.4574 8.87175C18.5536 9.11762 18.6674 9.48863 18.698 10.1711C18.7321 10.9088 18.7391 11.1301 18.7391 12.9983C18.7391 14.8664 18.7321 15.0886 18.698 15.8263C18.6674 16.5087 18.5528 16.8789 18.4574 17.1256C18.3444 17.4292 18.1653 17.7038 17.9332 17.9297C17.6882 18.1747 17.4555 18.3261 17.1283 18.453C16.8833 18.5493 16.5122 18.663 15.8289 18.6945C15.0912 18.7278 14.8699 18.7356 13.0009 18.7356C11.1319 18.7356 10.9096 18.7278 10.172 18.6945C9.4895 18.663 9.11937 18.5493 8.87262 18.453C8.56885 18.3403 8.2939 18.1616 8.06763 17.9297C7.83511 17.7037 7.65579 17.4287 7.54262 17.1248C7.44725 16.8789 7.33263 16.5079 7.302 15.8254C7.26875 15.0878 7.26175 14.8664 7.26175 12.9965C7.26175 11.1266 7.26875 10.907 7.302 10.1694C7.3335 9.48687 7.44725 9.11588 7.5435 8.86913C7.67037 8.54275 7.82262 8.30912 8.06763 8.06413C8.31262 7.81912 8.54537 7.66775 8.87262 7.54088C9.11937 7.44463 9.4895 7.33088 10.172 7.29938C10.8177 7.26963 11.068 7.26088 12.3726 7.26V7.26175ZM16.7371 8.42375C16.6268 8.42375 16.5176 8.44548 16.4157 8.48769C16.3138 8.52991 16.2212 8.59178 16.1432 8.66978C16.0652 8.74778 16.0033 8.84038 15.9611 8.9423C15.9189 9.04421 15.8971 9.15344 15.8971 9.26375C15.8971 9.37406 15.9189 9.48329 15.9611 9.5852C16.0033 9.68712 16.0652 9.77972 16.1432 9.85772C16.2212 9.93572 16.3138 9.99759 16.4157 10.0398C16.5176 10.082 16.6268 10.1038 16.7371 10.1038C16.9599 10.1038 17.1736 10.0153 17.3311 9.85772C17.4886 9.70019 17.5771 9.48653 17.5771 9.26375C17.5771 9.04097 17.4886 8.82731 17.3311 8.66978C17.1736 8.51225 16.9599 8.42375 16.7371 8.42375ZM13.0009 9.4055C12.5241 9.39806 12.0505 9.48555 11.6079 9.66287C11.1652 9.8402 10.7622 10.1038 10.4224 10.4384C10.0826 10.7729 9.81269 11.1717 9.62849 11.6116C9.44428 12.0515 9.34941 12.5236 9.34941 13.0004C9.34941 13.4773 9.44428 13.9494 9.62849 14.3893C9.81269 14.8291 10.0826 15.2279 10.4224 15.5625C10.7622 15.8971 11.1652 16.1607 11.6079 16.338C12.0505 16.5153 12.5241 16.6028 13.0009 16.5954C13.9446 16.5807 14.8447 16.1954 15.5069 15.5228C16.169 14.8503 16.5402 13.9443 16.5402 13.0004C16.5402 12.0566 16.169 11.1506 15.5069 10.478C14.8447 9.80545 13.9446 9.42022 13.0009 9.4055ZM13.0009 10.6664C13.3073 10.6664 13.6108 10.7267 13.8939 10.844C14.177 10.9613 14.4343 11.1332 14.651 11.3499C14.8677 11.5666 15.0396 11.8238 15.1569 12.107C15.2741 12.3901 15.3345 12.6935 15.3345 13C15.3345 13.3065 15.2741 13.6099 15.1569 13.893C15.0396 14.1762 14.8677 14.4334 14.651 14.6501C14.4343 14.8668 14.177 15.0387 13.8939 15.156C13.6108 15.2733 13.3073 15.3336 13.0009 15.3336C12.382 15.3336 11.7884 15.0878 11.3508 14.6501C10.9131 14.2125 10.6673 13.6189 10.6673 13C10.6673 12.3811 10.9131 11.7875 11.3508 11.3499C11.7884 10.9122 12.382 10.6664 13.0009 10.6664Z" fill="white"></path>
                                <defs>
                                    <lineargradient id="paint0_linear_956_791" x1="0" y1="0" x2="25.6176" y2="25.6176" gradientunits="userSpaceOnUse">
                                        <stop offset="0" stop-color="#405DE6"></stop>
                                        <stop offset="0.0928316" stop-color="#3A52C4"></stop>
                                        <stop offset="0.188792" stop-color="#833AB4"></stop>
                                        <stop offset="0.304378" stop-color="#C13584"></stop>
                                        <stop offset="0.444285" stop-color="#E1306C"></stop>
                                        <stop offset="0.608987" stop-color="#FF4545" stop-opacity="0.96"></stop>
                                        <stop offset="0.758196" stop-color="#F56040"></stop>
                                        <stop offset="0.893921" stop-color="#FCAF45"></stop>
                                        <stop offset="1" stop-color="#FFDC80"></stop>
                                    </lineargradient>
                                </defs>
                            </svg>
                            <h6>
                                <span>TRAVEL</span>
                                Instagram
                            </h6>
                        </a>
                        <img src="assets/image/standard/gallery-img4.png" alt="">
                    </div>
                </div>
                <div class="swiper-slide">
                    <div class="single-gallery-img">
                        <a href="#" class="view-btn">
                            <svg width="26" height="26" viewbox="0 0 26 26" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <rect width="26" height="26" rx="5" fill="url(#paint0_linear_956_792)"></rect>
                                <path d="M13 6C11.1004 6 10.8615 6.00875 10.1151 6.042C9.36875 6.077 8.86037 6.19425 8.415 6.3675C7.94761 6.54278 7.52437 6.81847 7.17513 7.17513C6.81847 7.52437 6.54278 7.94761 6.3675 8.415C6.19425 8.8595 6.07613 9.36875 6.042 10.1125C6.00875 10.8606 6 11.0986 6 13.0009C6 14.9014 6.00875 15.1394 6.042 15.8857C6.077 16.6313 6.19425 17.1396 6.3675 17.585C6.54688 18.0452 6.78575 18.4355 7.17513 18.8249C7.56363 19.2142 7.95388 19.454 8.41412 19.6325C8.86037 19.8057 9.36788 19.9239 10.1134 19.958C10.8606 19.9913 11.0986 20 13 20C14.9014 20 15.1385 19.9913 15.8857 19.958C16.6304 19.923 17.1405 19.8057 17.5859 19.6325C18.0529 19.4571 18.4759 19.1814 18.8249 18.8249C19.2142 18.4355 19.4531 18.0452 19.6325 17.585C19.8049 17.1396 19.923 16.6313 19.958 15.8857C19.9913 15.1394 20 14.9014 20 13C20 11.0986 19.9913 10.8606 19.958 10.1134C19.923 9.36875 19.8049 8.8595 19.6325 8.415C19.4572 7.94761 19.1815 7.52437 18.8249 7.17513C18.4756 6.81847 18.0524 6.54278 17.585 6.3675C17.1388 6.19425 16.6295 6.07613 15.8849 6.042C15.1376 6.00875 14.9005 6 12.9983 6H13ZM12.3726 7.26175H13.0009C14.8699 7.26175 15.0912 7.26788 15.8289 7.302C16.5114 7.33263 16.8824 7.44725 17.1291 7.54262C17.4555 7.6695 17.6891 7.82175 17.9341 8.06675C18.1791 8.31175 18.3305 8.5445 18.4574 8.87175C18.5536 9.11762 18.6674 9.48863 18.698 10.1711C18.7321 10.9088 18.7391 11.1301 18.7391 12.9983C18.7391 14.8664 18.7321 15.0886 18.698 15.8263C18.6674 16.5087 18.5528 16.8789 18.4574 17.1256C18.3444 17.4292 18.1653 17.7038 17.9332 17.9297C17.6882 18.1747 17.4555 18.3261 17.1283 18.453C16.8833 18.5493 16.5122 18.663 15.8289 18.6945C15.0912 18.7278 14.8699 18.7356 13.0009 18.7356C11.1319 18.7356 10.9096 18.7278 10.172 18.6945C9.4895 18.663 9.11937 18.5493 8.87262 18.453C8.56885 18.3403 8.2939 18.1616 8.06763 17.9297C7.83511 17.7037 7.65579 17.4287 7.54262 17.1248C7.44725 16.8789 7.33263 16.5079 7.302 15.8254C7.26875 15.0878 7.26175 14.8664 7.26175 12.9965C7.26175 11.1266 7.26875 10.907 7.302 10.1694C7.3335 9.48687 7.44725 9.11588 7.5435 8.86913C7.67037 8.54275 7.82262 8.30912 8.06763 8.06413C8.31262 7.81912 8.54537 7.66775 8.87262 7.54088C9.11937 7.44463 9.4895 7.33088 10.172 7.29938C10.8177 7.26963 11.068 7.26088 12.3726 7.26V7.26175ZM16.7371 8.42375C16.6268 8.42375 16.5176 8.44548 16.4157 8.48769C16.3138 8.52991 16.2212 8.59178 16.1432 8.66978C16.0652 8.74778 16.0033 8.84038 15.9611 8.9423C15.9189 9.04421 15.8971 9.15344 15.8971 9.26375C15.8971 9.37406 15.9189 9.48329 15.9611 9.5852C16.0033 9.68712 16.0652 9.77972 16.1432 9.85772C16.2212 9.93572 16.3138 9.99759 16.4157 10.0398C16.5176 10.082 16.6268 10.1038 16.7371 10.1038C16.9599 10.1038 17.1736 10.0153 17.3311 9.85772C17.4886 9.70019 17.5771 9.48653 17.5771 9.26375C17.5771 9.04097 17.4886 8.82731 17.3311 8.66978C17.1736 8.51225 16.9599 8.42375 16.7371 8.42375ZM13.0009 9.4055C12.5241 9.39806 12.0505 9.48555 11.6079 9.66287C11.1652 9.8402 10.7622 10.1038 10.4224 10.4384C10.0826 10.7729 9.81269 11.1717 9.62849 11.6116C9.44428 12.0515 9.34941 12.5236 9.34941 13.0004C9.34941 13.4773 9.44428 13.9494 9.62849 14.3893C9.81269 14.8291 10.0826 15.2279 10.4224 15.5625C10.7622 15.8971 11.1652 16.1607 11.6079 16.338C12.0505 16.5153 12.5241 16.6028 13.0009 16.5954C13.9446 16.5807 14.8447 16.1954 15.5069 15.5228C16.169 14.8503 16.5402 13.9443 16.5402 13.0004C16.5402 12.0566 16.169 11.1506 15.5069 10.478C14.8447 9.80545 13.9446 9.42022 13.0009 9.4055ZM13.0009 10.6664C13.3073 10.6664 13.6108 10.7267 13.8939 10.844C14.177 10.9613 14.4343 11.1332 14.651 11.3499C14.8677 11.5666 15.0396 11.8238 15.1569 12.107C15.2741 12.3901 15.3345 12.6935 15.3345 13C15.3345 13.3065 15.2741 13.6099 15.1569 13.893C15.0396 14.1762 14.8677 14.4334 14.651 14.6501C14.4343 14.8668 14.177 15.0387 13.8939 15.156C13.6108 15.2733 13.3073 15.3336 13.0009 15.3336C12.382 15.3336 11.7884 15.0878 11.3508 14.6501C10.9131 14.2125 10.6673 13.6189 10.6673 13C10.6673 12.3811 10.9131 11.7875 11.3508 11.3499C11.7884 10.9122 12.382 10.6664 13.0009 10.6664Z" fill="white"></path>
                                <defs>
                                    <lineargradient id="paint0_linear_956_792" x1="0" y1="0" x2="25.6176" y2="25.6176" gradientunits="userSpaceOnUse">
                                        <stop offset="0" stop-color="#405DE6"></stop>
                                        <stop offset="0.0928316" stop-color="#3A52C4"></stop>
                                        <stop offset="0.188792" stop-color="#833AB4"></stop>
                                        <stop offset="0.304378" stop-color="#C13584"></stop>
                                        <stop offset="0.444285" stop-color="#E1306C"></stop>
                                        <stop offset="0.608987" stop-color="#FF4545" stop-opacity="0.96"></stop>
                                        <stop offset="0.758196" stop-color="#F56040"></stop>
                                        <stop offset="0.893921" stop-color="#FCAF45"></stop>
                                        <stop offset="1" stop-color="#FFDC80"></stop>
                                    </lineargradient>
                                </defs>
                            </svg>
                            <h6>
                                <span>TRAVEL</span>
                                Instagram
                            </h6>
                        </a>
                        <img src="assets/image/standard/gallery-img5.png" alt="">
                    </div>
                </div>
                <div class="swiper-slide">
                    <div class="single-gallery-img">
                        <a href="#" class="view-btn">
                            <svg width="26" height="26" viewbox="0 0 26 26" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <rect width="26" height="26" rx="5" fill="url(#paint0_linear_956_793)"></rect>
                                <path d="M13 6C11.1004 6 10.8615 6.00875 10.1151 6.042C9.36875 6.077 8.86037 6.19425 8.415 6.3675C7.94761 6.54278 7.52437 6.81847 7.17513 7.17513C6.81847 7.52437 6.54278 7.94761 6.3675 8.415C6.19425 8.8595 6.07613 9.36875 6.042 10.1125C6.00875 10.8606 6 11.0986 6 13.0009C6 14.9014 6.00875 15.1394 6.042 15.8857C6.077 16.6313 6.19425 17.1396 6.3675 17.585C6.54688 18.0452 6.78575 18.4355 7.17513 18.8249C7.56363 19.2142 7.95388 19.454 8.41412 19.6325C8.86037 19.8057 9.36788 19.9239 10.1134 19.958C10.8606 19.9913 11.0986 20 13 20C14.9014 20 15.1385 19.9913 15.8857 19.958C16.6304 19.923 17.1405 19.8057 17.5859 19.6325C18.0529 19.4571 18.4759 19.1814 18.8249 18.8249C19.2142 18.4355 19.4531 18.0452 19.6325 17.585C19.8049 17.1396 19.923 16.6313 19.958 15.8857C19.9913 15.1394 20 14.9014 20 13C20 11.0986 19.9913 10.8606 19.958 10.1134C19.923 9.36875 19.8049 8.8595 19.6325 8.415C19.4572 7.94761 19.1815 7.52437 18.8249 7.17513C18.4756 6.81847 18.0524 6.54278 17.585 6.3675C17.1388 6.19425 16.6295 6.07613 15.8849 6.042C15.1376 6.00875 14.9005 6 12.9983 6H13ZM12.3726 7.26175H13.0009C14.8699 7.26175 15.0912 7.26788 15.8289 7.302C16.5114 7.33263 16.8824 7.44725 17.1291 7.54262C17.4555 7.6695 17.6891 7.82175 17.9341 8.06675C18.1791 8.31175 18.3305 8.5445 18.4574 8.87175C18.5536 9.11762 18.6674 9.48863 18.698 10.1711C18.7321 10.9088 18.7391 11.1301 18.7391 12.9983C18.7391 14.8664 18.7321 15.0886 18.698 15.8263C18.6674 16.5087 18.5528 16.8789 18.4574 17.1256C18.3444 17.4292 18.1653 17.7038 17.9332 17.9297C17.6882 18.1747 17.4555 18.3261 17.1283 18.453C16.8833 18.5493 16.5122 18.663 15.8289 18.6945C15.0912 18.7278 14.8699 18.7356 13.0009 18.7356C11.1319 18.7356 10.9096 18.7278 10.172 18.6945C9.4895 18.663 9.11937 18.5493 8.87262 18.453C8.56885 18.3403 8.2939 18.1616 8.06763 17.9297C7.83511 17.7037 7.65579 17.4287 7.54262 17.1248C7.44725 16.8789 7.33263 16.5079 7.302 15.8254C7.26875 15.0878 7.26175 14.8664 7.26175 12.9965C7.26175 11.1266 7.26875 10.907 7.302 10.1694C7.3335 9.48687 7.44725 9.11588 7.5435 8.86913C7.67037 8.54275 7.82262 8.30912 8.06763 8.06413C8.31262 7.81912 8.54537 7.66775 8.87262 7.54088C9.11937 7.44463 9.4895 7.33088 10.172 7.29938C10.8177 7.26963 11.068 7.26088 12.3726 7.26V7.26175ZM16.7371 8.42375C16.6268 8.42375 16.5176 8.44548 16.4157 8.48769C16.3138 8.52991 16.2212 8.59178 16.1432 8.66978C16.0652 8.74778 16.0033 8.84038 15.9611 8.9423C15.9189 9.04421 15.8971 9.15344 15.8971 9.26375C15.8971 9.37406 15.9189 9.48329 15.9611 9.5852C16.0033 9.68712 16.0652 9.77972 16.1432 9.85772C16.2212 9.93572 16.3138 9.99759 16.4157 10.0398C16.5176 10.082 16.6268 10.1038 16.7371 10.1038C16.9599 10.1038 17.1736 10.0153 17.3311 9.85772C17.4886 9.70019 17.5771 9.48653 17.5771 9.26375C17.5771 9.04097 17.4886 8.82731 17.3311 8.66978C17.1736 8.51225 16.9599 8.42375 16.7371 8.42375ZM13.0009 9.4055C12.5241 9.39806 12.0505 9.48555 11.6079 9.66287C11.1652 9.8402 10.7622 10.1038 10.4224 10.4384C10.0826 10.7729 9.81269 11.1717 9.62849 11.6116C9.44428 12.0515 9.34941 12.5236 9.34941 13.0004C9.34941 13.4773 9.44428 13.9494 9.62849 14.3893C9.81269 14.8291 10.0826 15.2279 10.4224 15.5625C10.7622 15.8971 11.1652 16.1607 11.6079 16.338C12.0505 16.5153 12.5241 16.6028 13.0009 16.5954C13.9446 16.5807 14.8447 16.1954 15.5069 15.5228C16.169 14.8503 16.5402 13.9443 16.5402 13.0004C16.5402 12.0566 16.169 11.1506 15.5069 10.478C14.8447 9.80545 13.9446 9.42022 13.0009 9.4055ZM13.0009 10.6664C13.3073 10.6664 13.6108 10.7267 13.8939 10.844C14.177 10.9613 14.4343 11.1332 14.651 11.3499C14.8677 11.5666 15.0396 11.8238 15.1569 12.107C15.2741 12.3901 15.3345 12.6935 15.3345 13C15.3345 13.3065 15.2741 13.6099 15.1569 13.893C15.0396 14.1762 14.8677 14.4334 14.651 14.6501C14.4343 14.8668 14.177 15.0387 13.8939 15.156C13.6108 15.2733 13.3073 15.3336 13.0009 15.3336C12.382 15.3336 11.7884 15.0878 11.3508 14.6501C10.9131 14.2125 10.6673 13.6189 10.6673 13C10.6673 12.3811 10.9131 11.7875 11.3508 11.3499C11.7884 10.9122 12.382 10.6664 13.0009 10.6664Z" fill="white"></path>
                                <defs>
                                    <lineargradient id="paint0_linear_956_793" x1="0" y1="0" x2="25.6176" y2="25.6176" gradientunits="userSpaceOnUse">
                                        <stop offset="0" stop-color="#405DE6"></stop>
                                        <stop offset="0.0928316" stop-color="#3A52C4"></stop>
                                        <stop offset="0.188792" stop-color="#833AB4"></stop>
                                        <stop offset="0.304378" stop-color="#C13584"></stop>
                                        <stop offset="0.444285" stop-color="#E1306C"></stop>
                                        <stop offset="0.608987" stop-color="#FF4545" stop-opacity="0.96"></stop>
                                        <stop offset="0.758196" stop-color="#F56040"></stop>
                                        <stop offset="0.893921" stop-color="#FCAF45"></stop>
                                        <stop offset="1" stop-color="#FFDC80"></stop>
                                    </lineargradient>
                                </defs>
                            </svg>
                            <h6>
                                <span>TRAVEL</span>
                                Instagram
                            </h6>
                        </a>
                        <img src="assets/image/standard/gallery-img6.png" alt="">
                    </div>
                </div>
                <div class="swiper-slide">
                    <div class="single-gallery-img">
                        <a href="#" class="view-btn">
                            <svg width="26" height="26" viewbox="0 0 26 26" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <rect width="26" height="26" rx="5" fill="url(#paint0_linear_956_794)"></rect>
                                <path d="M13 6C11.1004 6 10.8615 6.00875 10.1151 6.042C9.36875 6.077 8.86037 6.19425 8.415 6.3675C7.94761 6.54278 7.52437 6.81847 7.17513 7.17513C6.81847 7.52437 6.54278 7.94761 6.3675 8.415C6.19425 8.8595 6.07613 9.36875 6.042 10.1125C6.00875 10.8606 6 11.0986 6 13.0009C6 14.9014 6.00875 15.1394 6.042 15.8857C6.077 16.6313 6.19425 17.1396 6.3675 17.585C6.54688 18.0452 6.78575 18.4355 7.17513 18.8249C7.56363 19.2142 7.95388 19.454 8.41412 19.6325C8.86037 19.8057 9.36788 19.9239 10.1134 19.958C10.8606 19.9913 11.0986 20 13 20C14.9014 20 15.1385 19.9913 15.8857 19.958C16.6304 19.923 17.1405 19.8057 17.5859 19.6325C18.0529 19.4571 18.4759 19.1814 18.8249 18.8249C19.2142 18.4355 19.4531 18.0452 19.6325 17.585C19.8049 17.1396 19.923 16.6313 19.958 15.8857C19.9913 15.1394 20 14.9014 20 13C20 11.0986 19.9913 10.8606 19.958 10.1134C19.923 9.36875 19.8049 8.8595 19.6325 8.415C19.4572 7.94761 19.1815 7.52437 18.8249 7.17513C18.4756 6.81847 18.0524 6.54278 17.585 6.3675C17.1388 6.19425 16.6295 6.07613 15.8849 6.042C15.1376 6.00875 14.9005 6 12.9983 6H13ZM12.3726 7.26175H13.0009C14.8699 7.26175 15.0912 7.26788 15.8289 7.302C16.5114 7.33263 16.8824 7.44725 17.1291 7.54262C17.4555 7.6695 17.6891 7.82175 17.9341 8.06675C18.1791 8.31175 18.3305 8.5445 18.4574 8.87175C18.5536 9.11762 18.6674 9.48863 18.698 10.1711C18.7321 10.9088 18.7391 11.1301 18.7391 12.9983C18.7391 14.8664 18.7321 15.0886 18.698 15.8263C18.6674 16.5087 18.5528 16.8789 18.4574 17.1256C18.3444 17.4292 18.1653 17.7038 17.9332 17.9297C17.6882 18.1747 17.4555 18.3261 17.1283 18.453C16.8833 18.5493 16.5122 18.663 15.8289 18.6945C15.0912 18.7278 14.8699 18.7356 13.0009 18.7356C11.1319 18.7356 10.9096 18.7278 10.172 18.6945C9.4895 18.663 9.11937 18.5493 8.87262 18.453C8.56885 18.3403 8.2939 18.1616 8.06763 17.9297C7.83511 17.7037 7.65579 17.4287 7.54262 17.1248C7.44725 16.8789 7.33263 16.5079 7.302 15.8254C7.26875 15.0878 7.26175 14.8664 7.26175 12.9965C7.26175 11.1266 7.26875 10.907 7.302 10.1694C7.3335 9.48687 7.44725 9.11588 7.5435 8.86913C7.67037 8.54275 7.82262 8.30912 8.06763 8.06413C8.31262 7.81912 8.54537 7.66775 8.87262 7.54088C9.11937 7.44463 9.4895 7.33088 10.172 7.29938C10.8177 7.26963 11.068 7.26088 12.3726 7.26V7.26175ZM16.7371 8.42375C16.6268 8.42375 16.5176 8.44548 16.4157 8.48769C16.3138 8.52991 16.2212 8.59178 16.1432 8.66978C16.0652 8.74778 16.0033 8.84038 15.9611 8.9423C15.9189 9.04421 15.8971 9.15344 15.8971 9.26375C15.8971 9.37406 15.9189 9.48329 15.9611 9.5852C16.0033 9.68712 16.0652 9.77972 16.1432 9.85772C16.2212 9.93572 16.3138 9.99759 16.4157 10.0398C16.5176 10.082 16.6268 10.1038 16.7371 10.1038C16.9599 10.1038 17.1736 10.0153 17.3311 9.85772C17.4886 9.70019 17.5771 9.48653 17.5771 9.26375C17.5771 9.04097 17.4886 8.82731 17.3311 8.66978C17.1736 8.51225 16.9599 8.42375 16.7371 8.42375ZM13.0009 9.4055C12.5241 9.39806 12.0505 9.48555 11.6079 9.66287C11.1652 9.8402 10.7622 10.1038 10.4224 10.4384C10.0826 10.7729 9.81269 11.1717 9.62849 11.6116C9.44428 12.0515 9.34941 12.5236 9.34941 13.0004C9.34941 13.4773 9.44428 13.9494 9.62849 14.3893C9.81269 14.8291 10.0826 15.2279 10.4224 15.5625C10.7622 15.8971 11.1652 16.1607 11.6079 16.338C12.0505 16.5153 12.5241 16.6028 13.0009 16.5954C13.9446 16.5807 14.8447 16.1954 15.5069 15.5228C16.169 14.8503 16.5402 13.9443 16.5402 13.0004C16.5402 12.0566 16.169 11.1506 15.5069 10.478C14.8447 9.80545 13.9446 9.42022 13.0009 9.4055ZM13.0009 10.6664C13.3073 10.6664 13.6108 10.7267 13.8939 10.844C14.177 10.9613 14.4343 11.1332 14.651 11.3499C14.8677 11.5666 15.0396 11.8238 15.1569 12.107C15.2741 12.3901 15.3345 12.6935 15.3345 13C15.3345 13.3065 15.2741 13.6099 15.1569 13.893C15.0396 14.1762 14.8677 14.4334 14.651 14.6501C14.4343 14.8668 14.177 15.0387 13.8939 15.156C13.6108 15.2733 13.3073 15.3336 13.0009 15.3336C12.382 15.3336 11.7884 15.0878 11.3508 14.6501C10.9131 14.2125 10.6673 13.6189 10.6673 13C10.6673 12.3811 10.9131 11.7875 11.3508 11.3499C11.7884 10.9122 12.382 10.6664 13.0009 10.6664Z" fill="white"></path>
                                <defs>
                                    <lineargradient id="paint0_linear_956_794" x1="0" y1="0" x2="25.6176" y2="25.6176" gradientunits="userSpaceOnUse">
                                        <stop offset="0" stop-color="#405DE6"></stop>
                                        <stop offset="0.0928316" stop-color="#3A52C4"></stop>
                                        <stop offset="0.188792" stop-color="#833AB4"></stop>
                                        <stop offset="0.304378" stop-color="#C13584"></stop>
                                        <stop offset="0.444285" stop-color="#E1306C"></stop>
                                        <stop offset="0.608987" stop-color="#FF4545" stop-opacity="0.96"></stop>
                                        <stop offset="0.758196" stop-color="#F56040"></stop>
                                        <stop offset="0.893921" stop-color="#FCAF45"></stop>
                                        <stop offset="1" stop-color="#FFDC80"></stop>
                                    </lineargradient>
                                </defs>
                            </svg>
                            <h6>
                                <span>TRAVEL</span>
                                Instagram
                            </h6>
                        </a>
                        <img src="assets/image/standard/gallery-img7.png" alt="">
                    </div>
                </div>
                <div class="swiper-slide">
                    <div class="single-gallery-img">
                        <a href="#" class="view-btn">
                            <svg width="26" height="26" viewbox="0 0 26 26" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <rect width="26" height="26" rx="5" fill="url(#paint0_linear_956_795)"></rect>
                                <path d="M13 6C11.1004 6 10.8615 6.00875 10.1151 6.042C9.36875 6.077 8.86037 6.19425 8.415 6.3675C7.94761 6.54278 7.52437 6.81847 7.17513 7.17513C6.81847 7.52437 6.54278 7.94761 6.3675 8.415C6.19425 8.8595 6.07613 9.36875 6.042 10.1125C6.00875 10.8606 6 11.0986 6 13.0009C6 14.9014 6.00875 15.1394 6.042 15.8857C6.077 16.6313 6.19425 17.1396 6.3675 17.585C6.54688 18.0452 6.78575 18.4355 7.17513 18.8249C7.56363 19.2142 7.95388 19.454 8.41412 19.6325C8.86037 19.8057 9.36788 19.9239 10.1134 19.958C10.8606 19.9913 11.0986 20 13 20C14.9014 20 15.1385 19.9913 15.8857 19.958C16.6304 19.923 17.1405 19.8057 17.5859 19.6325C18.0529 19.4571 18.4759 19.1814 18.8249 18.8249C19.2142 18.4355 19.4531 18.0452 19.6325 17.585C19.8049 17.1396 19.923 16.6313 19.958 15.8857C19.9913 15.1394 20 14.9014 20 13C20 11.0986 19.9913 10.8606 19.958 10.1134C19.923 9.36875 19.8049 8.8595 19.6325 8.415C19.4572 7.94761 19.1815 7.52437 18.8249 7.17513C18.4756 6.81847 18.0524 6.54278 17.585 6.3675C17.1388 6.19425 16.6295 6.07613 15.8849 6.042C15.1376 6.00875 14.9005 6 12.9983 6H13ZM12.3726 7.26175H13.0009C14.8699 7.26175 15.0912 7.26788 15.8289 7.302C16.5114 7.33263 16.8824 7.44725 17.1291 7.54262C17.4555 7.6695 17.6891 7.82175 17.9341 8.06675C18.1791 8.31175 18.3305 8.5445 18.4574 8.87175C18.5536 9.11762 18.6674 9.48863 18.698 10.1711C18.7321 10.9088 18.7391 11.1301 18.7391 12.9983C18.7391 14.8664 18.7321 15.0886 18.698 15.8263C18.6674 16.5087 18.5528 16.8789 18.4574 17.1256C18.3444 17.4292 18.1653 17.7038 17.9332 17.9297C17.6882 18.1747 17.4555 18.3261 17.1283 18.453C16.8833 18.5493 16.5122 18.663 15.8289 18.6945C15.0912 18.7278 14.8699 18.7356 13.0009 18.7356C11.1319 18.7356 10.9096 18.7278 10.172 18.6945C9.4895 18.663 9.11937 18.5493 8.87262 18.453C8.56885 18.3403 8.2939 18.1616 8.06763 17.9297C7.83511 17.7037 7.65579 17.4287 7.54262 17.1248C7.44725 16.8789 7.33263 16.5079 7.302 15.8254C7.26875 15.0878 7.26175 14.8664 7.26175 12.9965C7.26175 11.1266 7.26875 10.907 7.302 10.1694C7.3335 9.48687 7.44725 9.11588 7.5435 8.86913C7.67037 8.54275 7.82262 8.30912 8.06763 8.06413C8.31262 7.81912 8.54537 7.66775 8.87262 7.54088C9.11937 7.44463 9.4895 7.33088 10.172 7.29938C10.8177 7.26963 11.068 7.26088 12.3726 7.26V7.26175ZM16.7371 8.42375C16.6268 8.42375 16.5176 8.44548 16.4157 8.48769C16.3138 8.52991 16.2212 8.59178 16.1432 8.66978C16.0652 8.74778 16.0033 8.84038 15.9611 8.9423C15.9189 9.04421 15.8971 9.15344 15.8971 9.26375C15.8971 9.37406 15.9189 9.48329 15.9611 9.5852C16.0033 9.68712 16.0652 9.77972 16.1432 9.85772C16.2212 9.93572 16.3138 9.99759 16.4157 10.0398C16.5176 10.082 16.6268 10.1038 16.7371 10.1038C16.9599 10.1038 17.1736 10.0153 17.3311 9.85772C17.4886 9.70019 17.5771 9.48653 17.5771 9.26375C17.5771 9.04097 17.4886 8.82731 17.3311 8.66978C17.1736 8.51225 16.9599 8.42375 16.7371 8.42375ZM13.0009 9.4055C12.5241 9.39806 12.0505 9.48555 11.6079 9.66287C11.1652 9.8402 10.7622 10.1038 10.4224 10.4384C10.0826 10.7729 9.81269 11.1717 9.62849 11.6116C9.44428 12.0515 9.34941 12.5236 9.34941 13.0004C9.34941 13.4773 9.44428 13.9494 9.62849 14.3893C9.81269 14.8291 10.0826 15.2279 10.4224 15.5625C10.7622 15.8971 11.1652 16.1607 11.6079 16.338C12.0505 16.5153 12.5241 16.6028 13.0009 16.5954C13.9446 16.5807 14.8447 16.1954 15.5069 15.5228C16.169 14.8503 16.5402 13.9443 16.5402 13.0004C16.5402 12.0566 16.169 11.1506 15.5069 10.478C14.8447 9.80545 13.9446 9.42022 13.0009 9.4055ZM13.0009 10.6664C13.3073 10.6664 13.6108 10.7267 13.8939 10.844C14.177 10.9613 14.4343 11.1332 14.651 11.3499C14.8677 11.5666 15.0396 11.8238 15.1569 12.107C15.2741 12.3901 15.3345 12.6935 15.3345 13C15.3345 13.3065 15.2741 13.6099 15.1569 13.893C15.0396 14.1762 14.8677 14.4334 14.651 14.6501C14.4343 14.8668 14.177 15.0387 13.8939 15.156C13.6108 15.2733 13.3073 15.3336 13.0009 15.3336C12.382 15.3336 11.7884 15.0878 11.3508 14.6501C10.9131 14.2125 10.6673 13.6189 10.6673 13C10.6673 12.3811 10.9131 11.7875 11.3508 11.3499C11.7884 10.9122 12.382 10.6664 13.0009 10.6664Z" fill="white"></path>
                                <defs>
                                    <lineargradient id="paint0_linear_956_795" x1="0" y1="0" x2="25.6176" y2="25.6176" gradientunits="userSpaceOnUse">
                                        <stop offset="0" stop-color="#405DE6"></stop>
                                        <stop offset="0.0928316" stop-color="#3A52C4"></stop>
                                        <stop offset="0.188792" stop-color="#833AB4"></stop>
                                        <stop offset="0.304378" stop-color="#C13584"></stop>
                                        <stop offset="0.444285" stop-color="#E1306C"></stop>
                                        <stop offset="0.608987" stop-color="#FF4545" stop-opacity="0.96"></stop>
                                        <stop offset="0.758196" stop-color="#F56040"></stop>
                                        <stop offset="0.893921" stop-color="#FCAF45"></stop>
                                        <stop offset="1" stop-color="#FFDC80"></stop>
                                    </lineargradient>
                                </defs>
                            </svg>
                            <h6>
                                <span>TRAVEL</span>
                                Instagram
                            </h6>
                        </a>
                        <img src="assets/image/standard/gallery-img8.png" alt="">
                    </div>
                </div>
            </div>
        </div>
    </div>
@endsection
